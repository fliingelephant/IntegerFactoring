# F269-D01 draft preregistration — adaptive Euclidean prefix search

## Status

This packet is an unfrozen sixth theory draft. Five earlier draft states
each received `KILL/REVISE`; all five failed verdicts remain attached to
their exact bytes. It is not registered. No F269 source has compiled. No
benchmark, corpus, or remote process has run.

Before any freeze, a sixth fresh hostile agent must try to kill the exact
algebra and this typed grammar. No source or runner may be written before that
theory kill passes. The later source must then pass an independent static
audit. The measured stress preflight must pass the readiness gate below.

F269-D01 is incompatible with an active F265 production or validation
...
## 5. Fixed draft expression language proposed for D01

This section remains provisional until the hostile kill and source audit.
...
## 6. No child-factor lane in D01

F269-D01 does not factor `K_a`, `J_a`, or any quotient corner. It invokes no
...
D01 omits them completely. The earlier provisional estimate of about
200,000 child factorizations is withdrawn. The exact D01 child-factor count
is zero.
...
`F269-D01.baselines.tsv` is ASCII with LF line endings, no BOM, no quoting,
...
`F269-D01.baseline_aggregates.tsv`; baseline records never share the selector
syntax-ID namespace.
...
The proposed uncompressed outputs are:
...
F269-D01.baseline_aggregates.tsv
...
The file `F269-D01.heldout.counterexamples.tsv` has exactly this header:
...
All other TSV header bytes, the executable, and schema sidecars are charged
to `manifests and sidecars`; exceeding that shared component cap is a
pre-production resource failure.
...
logs, sidecars, serialization temporaries, and compression temporaries, must
be below one dedicated evidence directory. Evidence symlinks, hard links,
sparse files, and output paths outside that directory are forbidden. Compiler
internal temporaries are not evidence and remain covered by the free-disk,
deadline, cgroup, and process limits. A process-shared checked writer keeps
an atomic sum of the current logical sizes of all live regular files. Before
every extending write it reserves the exact byte increment and refuses an
increment that would make the sum exceed
...
Truncation and rename update the same ledger; deletion cannot authorize a
missing required artifact. Only the failure-manifest writer can consume the
last `FAILURE_RESERVE` bytes. Shell stdout/stderr use checked log writers and
cannot bypass the ledger.

The runner independently executes GNU
`du -sb --apparent-size -- <evidence-directory>` after every file close,
before and after each production phase, before and after each compression,
and before final manifest closure. Every reading must be at most
`536870912`. It also computes the sum of `st_size` over every regular file
and requires it to equal the writer ledger; the `du` reading may be larger
because it includes directory entries but can never exceed the cap. A
mismatch or excess is a resource failure. Compression handles one artifact
at a time: its temporary compressed file and the original both count until
the authenticated replacement closes. No batch of duplicate compressed
copies is permitted.
...
the packet fails before compile. There is no watchdog-only fallback.
...
Before source implementation or any freeze:

1. a sixth different fresh hostile theory agent must pass both revised draft
   files.

After that pass and before any dynamic cohort step:
