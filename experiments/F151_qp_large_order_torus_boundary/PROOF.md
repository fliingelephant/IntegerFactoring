# Proof of the F151 large-order and torus boundary
...
## 2. Local injectivity and null sign screens
...
`alpha^(2(e-f))=1 modulo r`, again with exponent below `B`.
...
the stated carry qualification is necessary.
...
Let `R=exp(Cd)` be a radius large enough to cover the stated norm bound.
The number of integer points in the enclosing coordinate box is at most
...
Finally, the defining identity for Chebyshev polynomials gives
