...
readonly -a frozen_packet_files=(
  ALGEBRA.md
  PREREGISTRATION.md
  search.cpp
  remote_run.sh
...
readonly -a packet_evidence_files=(
  "${frozen_packet_files[@]}"
  FROZEN.sha256
...
verify_frozen_packet_list() {
  local expected observed manifest_lines
  expected=$(printf '%s\n' "${frozen_packet_files[@]}" | LC_ALL=C sort)
  observed=$(
    awk '
      NF != 2 || length($1) != 64 || $1 ~ /[^0-9a-f]/ {exit 91}
      {print $2}
    ' FROZEN.sha256 | LC_ALL=C sort
  )
  manifest_lines=$(wc -l <FROZEN.sha256)
  [[ $manifest_lines -eq ${#frozen_packet_files[@]} ]]
  [[ $observed == "$expected" ]]
}
  ...
  bytes=$(
    {
      ...
      stat -c '%s' "${packet_evidence_files[@]}"
...
(
  ...
  timeout "${remaining}s" nice -n 15 bash -o pipefail -c '
    archive=$1
    shift
    tar -cf - "$@" output logs preflight | zstd -q -T1 -3 -o "$archive"
