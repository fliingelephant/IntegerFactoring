# F270-D03 static hostile audit request
...
D02 repaired every D01 resource blocker, but D02 launch was prohibited by
audit SHA-256
`1a5b62fec6ab10c411a1f60270f866dd9bd6d8a3ab8b40919032758344c23739`
because its process classifier could miss a generic legacy runner and its
ancestor walk could silently truncate. Confirm that D03 preserves the
approved D02 mathematics and resource envelope and repairs exactly that
process-firewall blocker.
...
16. The generic exclusive production lock and complete bounded `/proc` scan
    are noninteractive. The ancestor walk reaches PID 1 and fails closed on
    its finite depth/byte caps. Exclusions bind exact `(PID,starttime,PPID)`
    identities and contain only that full chain plus the proved direct-child
    deadline watchdog. Two numeric-PID-sorted snapshots record the complete
    command line, executable path, and working directory. PID lists before,
    between, and after remain identical; the two snapshots are byte-identical;
    and all read, identity, count, and byte inconsistencies fail closed. Every
    nonexcluded `remote_run` or `remote_run.sh` rejects without needing an
    F-number, mode, argument order, or path spelling. Semantic mode/workload
    detection remains. Both full snapshots, exact exclusions, their hashes,
    and literal firewall PASS enter archived and final evidence. The scan
    occurs before any workload child, and the lock remains authoritative for
    cooperating packets until finalization.
