# F260-D02 V2 algebra — exact public integer-source scores
...
V2 repairs the D01 score ambiguity. It ranks factored annihilators at two
operational certified states:
...
V2 forbids the D01 quadratic product-over-all-other-primaries loop.
...
V2 computes the exact analytic distribution
...
V2 records (9), `H(W)=max(alpha_p,alpha_q)`, both residuals through their
...
divides `N-1`. For side `i`, V2 therefore scores only
...
V2 records each primary contribution and `min(C_p/log2(s_p^perp),
...
in lowest terms. V2 constructs each endpoint by short signed interval
