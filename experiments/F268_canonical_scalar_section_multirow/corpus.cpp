...
bool marker_case(const std::string& split, int bits, int index, int retry,
  ...
  u64 p = 0, q = 0;
  try {
    p = random_prime_class(rng, bits, 24, 13, 128);
    q = random_prime_class(rng, bits, 72, 11, 128);
...
void append_marker(const std::string& split, const int* bits, int bit_count,
  ...
  for (int bi = 0; bi < bit_count; ++bi) {
    ...
    for (int index = 0; index < 2; ++index) {
      bool accepted = false;
      for (int retry = 0; retry < 4096 && !accepted; ++retry) {
        Case c;
    ...
    if (found < 2) shortfalls.emplace_back(split, bits[bi], 2 - found);
  }
}
...
std::string shortfall_bytes(
  ...
  for (const auto& [split, bits, missing] : shortfalls)
    out << "F268-D01\t" << split << "\tmarker-control\t" << bits << '\t'
        << missing << '\n';
  return out.str();
}
...
void generate(const std::string& directory) {
  ...
  write_new(join_path(directory, "F268-D01.discovery.public.tsv"),
            discovery_public, 0644);
  write_new(join_path(directory, "F268-D01.heldout.public.tsv"),
            heldout_public, 0644);
  write_new(join_path(directory, "F268-D01.discovery.labels.tsv"),
            discovery_labels, 0600);
  write_new(join_path(directory, "F268-D01.heldout.labels.tsv"),
            heldout_labels, 0600);
  write_new(join_path(directory, "F268-D01.marker_shortfalls.tsv"),
