# F266-D02 preregistration — split-quadratic discriminant-lift search

F266-D02 is the narrow successor to immutable failed packet F266-D01. The
algebraic grammar, family IDs, word source, matrix caps, lift levels, tickets,
decoys, cohorts, and ranking tuple are unchanged. D02 repairs only the shared
canonical lift, stage chronology, evidence authentication, and whole-packet
resource gates.
...
IDs. The parser requires those IDs, in order, to equal the first four IDs in
the authenticated rank rows and validates every rank-field syntax. It reads no
...
baseline sums. Equal integer values count once.
...
every heldout size/shape cell for one selected family. Other outcomes are
inconclusive.
...
steps, all serialized evidence bytes, and conservative eight-worker full-run
wall, RSS, and output projections. It stops unless projected wall time is at
most 12,600 seconds, projected RSS is at most 3.5 GiB, projected output is at
most 900 MiB, and resource rejections are zero.
...
remaining wall time, 4 GiB, one GiB per output file, and one thread. An
external compression manifest records the archive byte count, line count, and
SHA-256.
...
The F266-D01 FAIL audit remains attached only to D01. It authorizes no D02
step. D02 needs a fresh independent audit of its own frozen bytes.
