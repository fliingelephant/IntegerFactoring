...
refuse_overlap() {
  if ps -eo args= | grep -E '[F]265|[f]265' >/dev/null; then
...
DISC_PUBLIC="$TARGET/corpus/F268-D04.discovery.public.tsv"
HELD_PUBLIC="$TARGET/corpus/F268-D04.heldout.public.tsv"
DISC_LABELS="$TARGET/corpus/F268-D04.discovery.labels.tsv"
HELD_LABELS="$TARGET/corpus/F268-D04.heldout.labels.tsv"
...
run_phase 60 nice -n 15 "$TARGET/bin/f268_search" --gate \
  ...
  "$TARGET/preflight/banks/F268-D04.preflight.metrics.tsv" \
...
SELECTION="$TARGET/discovery/F268-D04.selection.tsv"
...
run_phase 3600 nice -n 15 "$TARGET/bin/f268_search" --validate \
  ...
  "$TARGET/discovery/F268-D04.discovery.evidence.tsv" \
...
run_phase 3600 nice -n 15 "$TARGET/bin/f268_search" --validate \
  ...
  "$TARGET/heldout/F268-D04.heldout.evidence.tsv" \
...
run_phase 300 nice -n 15 "$TARGET/bin/f268_label_audit" --audit \
  "$DISC_PUBLIC" "$DISC_LABELS" "$TARGET/discovery/F268-D04.discovery.banks.tsv" \
  "$TARGET/corpus/F268-D04.marker_shortfalls.tsv" \
  "$TARGET/F268-D04.discovery.label_audit.tsv" \
  >"$TARGET/logs/discovery_label_audit.log" 2>&1
run_phase 300 nice -n 15 "$TARGET/bin/f268_label_audit" --audit \
  "$HELD_PUBLIC" "$HELD_LABELS" "$TARGET/heldout/F268-D04.heldout.banks.tsv" \
  "$TARGET/corpus/F268-D04.marker_shortfalls.tsv" \
  "$TARGET/F268-D04.heldout.label_audit.tsv" \
...
find "$TARGET" -type f ! -name 'F268-D04.final.sha256' -print0 | sort -z | \
  xargs -0 sha256sum >"$TARGET/F268-D04.final.sha256"
