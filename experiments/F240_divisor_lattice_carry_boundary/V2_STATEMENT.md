# F240 V2 candidate — the factored `N-1` divisor lattice has an exact centered-carry escape and a sharp divisor-only P205 boundary

## Status and scope

V2 repairs the exact decoder endpoint found by the V1 hostile audit.  Every
direct factor-bank claim now requires `(a,b)!=(0,0)`.  The carry algebra,
`d=2` offset, divisor-only grammar, and additive-selector boundary are
otherwise unchanged.
...
A decoder first requires `a,b>=0` and then rejects `(a,b)=(0,0)`.  For
`b>0`, it requires a nonnegative square discriminant, enumerates the two
quadratic-formula values, and returns only an integer `X` with `1<X<N` and
`X | N`.  For
`b=0`, the nondegeneracy gate gives `a>0`; the decoder requires `T!=0`,
solves the linear equation `X=aN/T`, and applies the same integer,
proper-range, and exact-divisibility checks.  Thus a public guessed tuple
`(B,u,a,b,c)` with `B | M` and `(a,b)!=(0,0)` is safely checked using
(2)--(4).  False tuples are harmless.

For a true centered tuple, `a,b>=0`, and `b=0` implies `a=0` because
`p<q`.  Hence every true nondegenerate tuple has `b>0` and the quadratic
decoder contains `p`.  The excluded endpoint is real: at `B=M,u=1`, one
has

\[
a=b=0,\qquad x=p,\qquad y=q,\qquad c=1,\qquad T=0,
\]

so (3) is the zero polynomial and gives no factor.  Other zero-center or
zero-residue cases are nondegenerate and safe.  If `a=0<b`, equation (3)
has roots `0,p`, and the proper-factor check selects `p`.  At `B=1`, both
centered residues are zero but `a=up,b=uq>0`, and (3) has the repeated root
`p`.
...
If both centers `a,b` and the carry `c` are in quasipolynomial banks, and
the correct tuple satisfies `(a,b)!=(0,0)`, Theorem A instead gives a
direct verified factor bank.  A bank may safely include degenerate tuples;
the decoder simply skips them.
...
2. a quasipolynomial bank contains both centers and a short carry at a
   true tuple with `(a,b)!=(0,0)`;
