...
struct BankResult {
  ...
  std::string error;
  std::vector<Row> rows;
  std::vector<Relation> saved_relations;
...
void record_factor(BankResult& result, const cpp_int& candidate, const cpp_int& N,
...
bool screened_unit(const cpp_int& value, const cpp_int& N, BankResult& result,
  ...
  if (g == 1) return true;
  if (g < N) record_factor(result,g,N,channel);
  return false;
}
...
Row make_row(const cpp_int& N, const BaseSource& base, const Family& family,
             const WordMatrix& word, int level, Rng& gauge, BankResult& result) {
  cpp_int m=N;
  if (level==2) m*=N;
  cpp_int a=base.a,r=base.r,s=base.s;
  if (level==2) {
    a += N*random_below(gauge,N);
    r += N*random_below(gauge,N);
    s += N*random_below(gauge,N);
  }
  cpp_int A0=modz(a,m);
  ...
  for (const auto& item : std::array<std::pair<cpp_int,std::string>,5>{{
         {A0,"BASE_COEFFICIENT_A"},{B0,"BASE_COEFFICIENT_B"},
         {C0,"BASE_COEFFICIENT_C"},{delta0,"BASE_DISCRIMINANT"},
         {root,"SUPPLIED_DISCRIMINANT_ROOT"}}})
    record_factor(result,item.first,N,item.second);
  ...
  for (const auto& item : std::array<std::pair<cpp_int,std::string>,4>{{
         {A,"TRANSFORMED_COEFFICIENT_A"},{B,"TRANSFORMED_COEFFICIENT_B"},
         {C,"TRANSFORMED_COEFFICIENT_C"},{D,"TRANSFORMED_DISCRIMINANT"}}})
    record_factor(result,item.first,N,item.second);
  ...
  for (const auto& item : std::array<std::pair<cpp_int,std::string>,4>{{
         {pr.numerator,"PROJECTIVE_ROOT_NUMERATOR"},{pr.denominator,"PROJECTIVE_ROOT_DENOMINATOR"},
         {ps.numerator,"PROJECTIVE_ROOT_NUMERATOR"},{ps.denominator,"PROJECTIVE_ROOT_DENOMINATOR"}}})
    record_factor(result,item.first,N,item.second);
  ...
  record_factor(result,projective_delta,N,"PROJECTIVE_ROOT_DIFFERENCE");
  Row row;
  ...
  for (int base_id=0;base_id<2;++base_id) {
    ...
    for (const auto& word:matrices) {
      ...
      for (int level=1;level<=2;++level) {
        Rng gauge(hash_cpp(N,MASTER_SEED ^ mix64(static_cast<u64>(family.id*100000+
                      base_id*1000+level*101+word.length*17))) ^
                  mix64(hash_cpp(cpp_int(word.matrix.a*131+word.matrix.b*17+
                                         word.matrix.c*7+word.matrix.d),0x4741554745ULL)));
        Row row=make_row(N,base,family,word,level,gauge,result);
...
void screen_pair_channels(const cpp_int& N, BankResult& result) {
...
void screen_carries(const cpp_int& N, BankResult& result) {
  for (const Row& row:result.rows) {
    ...
    record_factor(result,row.carry,N,"INVARIANT_CARRY",true);
    if (!before&&result.any_factor) result.strict_carry=true;
  }
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      const Row& x=result.rows[i];const Row& y=result.rows[j];
      ...
      ++result.carry_tests;
      bool before=result.any_factor;
      record_factor(result,quotient,N,x.level==y.level ?
                    "SAME_LEVEL_CARRY_QUOTIENT":"CROSS_LEVEL_CARRY_QUOTIENT",true);
      if (!before&&result.any_factor) result.strict_carry=true;
    }
  }
}
...
void screen_singletons_and_square_classes(const cpp_int& N, BankResult& result) {
  for (const Row& row:result.rows) {
    ...
    if (cls=="USEFUL") {
      ...
      record_factor(result,gm,N,row.identity?"BASE_SINGLETON_MINUS":"ORBIT_SINGLETON_MINUS");
      record_factor(result,gp,N,row.identity?"BASE_SINGLETON_PLUS":"ORBIT_SINGLETON_PLUS");
      if (!prior) {
  ...
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      ...
      if (cls=="USEFUL") {
        bool prior=result.any_factor;
        ++result.useful_square_multiples;
        record_factor(result,gm,N,"SQUARE_MULTIPLE_MINUS");
        record_factor(result,gp,N,"SQUARE_MULTIPLE_PLUS");
        if (!prior) result.strict_square_multiple=true;
      }
    }
  }
}
  ...
  try {
    ...
    for (const auto& bits:decoder.kernel) {
      ...
      if (relation.root_class=="GLOBAL_PLUS") ++result.global_plus;
        ...
        if (!prior) ++result.strict_multirow;
        record_factor(result,relation.gcd_minus,N,"MULTIROW_P66_MINUS");
        record_factor(result,relation.gcd_plus,N,"MULTIROW_P66_PLUS");
      }
      if (result.saved_relations.size()<2 && relation.root_class=="USEFUL")
        result.saved_relations.push_back(std::move(relation));
    }
  } catch (const std::exception& error) {
    result.eligible=false;result.error=error.what();
  }
  return result;
}
...
std::string corpus_bytes(const std::vector<Case>& cases) {
  ...
  for (const Case& c:cases)
    out<<"F266-D01\t"<<c.split<<'\t'<<c.shape<<'\t'<<c.factor_bits<<'\t'
       <<c.index<<'\t'<<c.N<<'\n';
  return out.str();
}
...
std::string bank_bytes(const std::vector<Case>& cases,const std::vector<Evaluated>& rows) {
  ...
  out<<"split\tshape\tfactor_bits\tcase_index\tN\tfamily\teligible\tresource_reject"
       ...
       "\tuniform_principal_baseline_num\tuniform_principal_baseline_den\terror\n";
...
std::string certificate_bytes(const std::vector<Case>& cases,
  ...
  for (const Evaluated& evaluated:rows) {
    ...
    const Case& c=cases[evaluated.case_index];const BankResult& b=evaluated.bank;
    if (b.first_factor>1) {
      out<<"{\"version\":\"F266-D01\",\"split\":"<<json_string(c.split)
         <<",\"shape\":"<<json_string(c.shape)<<",\"factor_bits\":"<<c.factor_bits
         <<",\"index\":"<<c.index<<",\"N\":"<<json_string(c.N.convert_to<std::string>())
         <<",\"family\":"<<evaluated.family<<",\"kind\":\"DIRECT_OR_CARRY\",\"channel\":"
         <<json_string(b.first_channel)<<",\"candidate\":"
         <<json_string(b.first_candidate.convert_to<std::string>())<<",\"factor\":"
         <<json_string(b.first_factor.convert_to<std::string>())<<",\"factor_label\":"
         <<json_string(evaluated.factor_label)<<"}\n";
      ++saved[evaluated.family];
      if (saved[evaluated.family]>=MAX_CERTIFICATES) continue;
    }
    for (const Relation& relation:b.saved_relations) {
      if (saved[evaluated.family]>=MAX_CERTIFICATES) break;
      out<<"{\"version\":\"F266-D01\",\"split\":"<<json_string(c.split)
         <<",\"shape\":"<<json_string(c.shape)<<",\"factor_bits\":"<<c.factor_bits
         <<",\"index\":"<<c.index<<",\"N\":"<<json_string(c.N.convert_to<std::string>())
         <<",\"family\":"<<evaluated.family<<",\"kind\":\"MULTIROW_P66\",\"label\":"
         <<json_string(relation.label)<<",\"root_class\":"<<json_string(relation.root_class)
         <<",\"exact_root\":"<<json_string(relation.exact_root.convert_to<std::string>())
         <<",\"supplied_root\":"<<json_string(relation.supplied_root.convert_to<std::string>())
         <<",\"normalized_root\":"<<json_string(relation.normalized_root.convert_to<std::string>())
         <<",\"gcd_minus\":"<<json_string(relation.gcd_minus.convert_to<std::string>())
         <<",\"gcd_plus\":"<<json_string(relation.gcd_plus.convert_to<std::string>())
         <<",\"rows\":[";
      for (std::size_t k=0;k<relation.columns.size();++k) {
        if (k) out<<',';
        const Row& row=b.rows[relation.columns[k]];
        out<<"{\"column\":"<<relation.columns[k]<<",\"base\":"<<row.base_id
...
Selection parse_selection(const std::string& bytes,const std::string& expected_digest) {
  ...
  require(bool(std::getline(input,line))&&line=="F266-D01-selection-v1","selection header");
  ...
  require(bool(std::getline(corpus,digest,'\t'))&&digest.size()==64,"selection corpus digest");
  require(bool(std::getline(input,line))&&line.rfind("family\trank\tid\t",0)==0,
...
std::string lead_gate_bytes(const std::vector<Case>& cases,
  ...
  u64 strict_multirow=0,strict_carry=0,strict_orbit=0,strict_square=0,eligible=0,intended=0;
  std::set<int> multi_sizes,carry_sizes;std::set<std::string> multi_shapes,carry_shapes;
  std::map<std::tuple<int,int,std::string>,u64> eligible_cell;
  struct Baseline {u64 banks=0,base=0,orbit=0;cpp_int num=0,den=1;};
  std::map<std::pair<int,std::string>,Baseline> baselines;
  for (const Evaluated& evaluated:rows) {
    const Case& c=cases[evaluated.case_index];const BankResult& b=evaluated.bank;
    ++intended;eligible+=b.eligible;
    if (b.eligible) ++eligible_cell[{evaluated.family,c.factor_bits,c.shape}];
    if (b.strict_multirow) {
      ++strict_multirow;multi_sizes.insert(c.factor_bits);multi_shapes.insert(c.shape);
    }
    if (b.strict_carry&&c.factor_bits>=48) {
      ++strict_carry;carry_sizes.insert(c.factor_bits);carry_shapes.insert(c.shape);
    }
    ...
    ++base.banks;base.base+=b.useful_base_singletons;base.orbit+=b.useful_orbit_singletons;
    auto sum=add_fraction(base.num,base.den,cpp_int(2*b.tested_rows),c.N);
    base.num=sum.first;base.den=sum.second;
  }
  bool hostile_multi=multi_shapes.count("safe-safe")||multi_shapes.count("neighbor");
  bool strong=strict_multirow>=2&&multi_sizes.size()>=2&&hostile_multi;
  bool carry=strict_carry>=2&&carry_sizes.size()>=2&&carry_shapes.size()>=2;
  bool cell_coverage=true;
  for (int bits:HELDOUT_BITS) for (const char* shape:SHAPES) {
    bool one=false;
    for (int family:selected)
      if (eligible_cell[{family,bits,shape}]>=24) one=true;
    cell_coverage&=one;
  }
  bool no_gain=strict_multirow==0&&strict_carry==0&&strict_orbit==0&&strict_square==0;
  ...
  out<<"metric\tvalue\nstrong_orbit_lead\t"<<strong<<"\ncarry_lead\t"<<carry
     ...
     <<"\nstrict_carry_banks_48plus\t"<<strict_carry<<"\nstrict_orbit_singleton_banks\t"
     <<strict_orbit<<"\nstrict_square_multiple_banks\t"<<strict_square
     <<"\neligible\t"<<eligible<<"\nintended\t"<<intended<<"\ncell_coverage\t"<<cell_coverage<<"\n";
  out<<"cell\tfactor_bits\tshape\tbanks\tuseful_base_singletons\tuseful_orbit_singletons"
       "\tuniform_principal_baseline_num\tuniform_principal_baseline_den\n";
  for (const auto& item:baselines) {
    const Baseline& b=item.second;
    out<<"cell\t"<<item.first.first<<'\t'<<item.first.second<<'\t'<<b.banks<<'\t'
       <<b.base<<'\t'<<b.orbit<<'\t'<<b.num<<'\t'<<b.den<<'\n';
  }
  return out.str();
}
...
std::set<std::string> parse_discovery_corpus(const std::string& bytes) {
  ...
  while (std::getline(input,line)) {
    ...
    std::istringstream row(line);std::vector<std::string> fields;std::string field;
    while (std::getline(row,field,'\t')) fields.push_back(field);
    require(fields.size()==6&&fields[0]=="F266-D01"&&fields[1]=="discovery",
...
void run_discovery(const std::string& directory,int workers) {
  ...
  const std::string corpus_name="F266-D01.discovery.corpus.tsv";
  const std::string corpus_sha_name="F266-D01.discovery.corpus.sha256";
  const std::string bank_name="F266-D01.discovery.banks.tsv";
  const std::string family_name="F266-D01.discovery.families.tsv";
  const std::string certificate_name="F266-D01.discovery.certificates.jsonl";
  const std::string selection_name="F266-D01.selection.tsv";
  const std::string selection_sha_name="F266-D01.selection.sha256";
  ...
  std::vector<std::string> names{corpus_name,corpus_sha_name,bank_name,family_name,
                                 certificate_name,selection_name,selection_sha_name};
  write_text(join_path(directory,"F266-D01.discovery.manifest.tsv"),manifest_bytes(directory,names));
...
void run_heldout(const std::string& directory,const std::string& selection_path,
  ...
  auto start=std::chrono::steady_clock::now();
  for (const char* name:{"F266-D01.discovery.corpus.tsv","F266-D01.discovery.corpus.sha256",
                         "F266-D01.discovery.banks.tsv","F266-D01.discovery.families.tsv",
                         "F266-D01.discovery.certificates.jsonl","F266-D01.selection.tsv",
                         "F266-D01.selection.sha256","F266-D01.discovery.manifest.tsv"})
  ...
  const std::string corpus_name="F266-D01.heldout.corpus.tsv";
  const std::string bank_name="F266-D01.heldout.banks.tsv";
  const std::string family_name="F266-D01.heldout.families.tsv";
  const std::string certificate_name="F266-D01.heldout.certificates.jsonl";
  const std::string lead_name="F266-D01.heldout.lead_gate.tsv";
  write_text(join_path(directory,corpus_name),corpus);
  write_text(join_path(directory,bank_name),banks);
  write_text(join_path(directory,family_name),families);
  write_text(join_path(directory,certificate_name),certificates);
  write_text(join_path(directory,lead_name),lead);
  std::vector<std::string> names{corpus_name,bank_name,family_name,certificate_name,lead_name};
  write_text(join_path(directory,"F266-D01.heldout.manifest.tsv"),manifest_bytes(directory,names));
...
void run_preflight(const std::string& output_path,int workers) {
  ...
  u64 pairs=0,resultants=0,steps=0,row_count=0,resource_reject=0;
  ...
  u64 projected_output=static_cast<u64>(sample_banks.size()*full_tasks/
                                        std::max<double>(1,rows.size())*2.0+16*1024*1024);
  bool pass=projected_wall<=12600.0&&usage.ru_maxrss<=3670016&&
            projected_output<=943718400&&resource_reject==0;
  std::ostringstream json;
  json<<"{\"version\":\"F266-D01\",\"pass\":"<<(pass?"true":"false")
      ...
      <<",\"maxrss_kib\":"<<usage.ru_maxrss<<",\"rows\":"<<row_count
