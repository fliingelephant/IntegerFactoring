# F264-D02 V2 preregistration — noncommutative matrix-word lift search

## Status and purpose

This packet freezes a repaired finite C++17 discovery experiment. V1 and its
hostile **FAIL** audit remain byte-for-byte unchanged. V2 searches public
...
complete integer `N` and the constants in `V2_symbolic_search.cpp`. In each
...
identity in `V2_ALGEBRA.md`. The executable aborts on a failed divisibility
test.
...
retain the fixed number with smallest `(key,index)`. The following caps are per
profile:
...
the resource envelope. V2 does not claim to do that. Instead, pair and triple
...
`V2_ALGEBRA.md`; the two coordinate products inside each matrix product are
...
`N`. This can change its gcd, as `N=15,a=45` demonstrates. V2 claims only
...
14 original-basis and public-conjugate-basis proposals in `V2_ALGEBRA.md`; no
square-root oracle is assumed. Report the exact decimal accumulated certified
...
`V2_symbolic_search.cpp`. Safe-safe is the hostile cohort.
...
The runner checks processes before compile, self-test, benchmark, describe,
discovery, held-out, report validation, and compression. A two-second monitor
...
each material stage. The runner validates exact row counts, 2,541 TSV columns,
JSON dimensions, named decoys, and split labels before compression.
