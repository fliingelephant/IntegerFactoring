# F259-D02 algebra — full Pell lifts and bounded symbolic atoms
...
### Zero-carry semantics

The formulas above hold without a `c != 0` hypothesis. If `c=0`, the full
second carry is the exact zero

\[
E_{r\mid h,i}=0,
\]

but its displayed core is still a defined public integer. It is
`3z` for `r=3` and
...
for `r=5`. F259-D02 evaluates the displayed formula directly for every
admitted composition. It never tries to recover the core by division by
`4Dc^2`. It separately checks the exact identity

\[
E_{r\mid h,i}=4Dc^2\,\operatorname{core}_{r\mid h,i}
\]

for all `c`, including zero.

The first-carry control is exactly

\[
D c(2z+cN).
\]

It is recorded as a zero atom when `c=0`; it is never replaced by a unit.
For target multiplier 15, the sum and difference of the `(r,h)=(5,3)` and
`(3,5)` cores are recorded whenever `15i` is in the frozen window, including
when either inner carry is zero.

## Exact cocycle feature basis

For `c_{ij}=(a_{ij},b_{ij})`, the real coordinate of the associativity
cocycle has exactly the ordered six-term basis
...
The imaginary coordinate has exactly the ordered six-term basis
...
These twelve terms are the complete F259-D02 identity-mining basis. Products
of two carry coordinates are not in this basis because they do not occur in
the displayed cocycle expression.
