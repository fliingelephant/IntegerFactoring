...
SOURCE = HERE / "stress_independent_all_pairs.py"
...
def main() -> int:
    command = [sys.executable, str(SOURCE), "--workers", "3", "--checkpoint", str(ATTEMPT)]
    try:
    ...
    if parsed.get("status") not in {"PASS", "FAIL"} or parsed.get("completed_cases") != SELECTED_CASES:
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
