...
## 4. Recursive accounting and the residue terminal
...
Thus complete recursive factorization of \(K\) is the single possible
\((n-1)\)-bit child. The floor definition of \(B\) gives
...
so \(E\) has at most \(n/2+O(1)\) bits. The same bound may be used for any
fixed or numerical-QP number of comparable auxiliary children. Therefore a
numerical-QP inverse-box selector after the complete factorization of \(K\)
has recurrence (6). P183 proves that this one-decrement-spine recurrence is
numerical QP. This is the one-child recursion correction; no second
near-size branch has been introduced.
