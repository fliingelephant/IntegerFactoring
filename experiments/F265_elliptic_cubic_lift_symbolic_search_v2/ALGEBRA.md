...
This file fixes the exact algebra for immutable F265-D02 before any search
result exists. It is a candidate packet. It is not a factoring theorem.

F265-D02 preserves the V1 cubic-row and square-class algebra. It repairs only
the source boundary, complete-bank rule, opaque-block evidence, chord-pattern
evidence, factor-label firewall, and resource accounting. It does not add an
elliptic family or a relation grammar.
...
An orbit that cannot reach its complete frozen scalar range without a proper
factor is not decoded. This includes a full nonunit row root and any affine
global exception not resolved as the point at infinity. It is reported as
`BANK_SKIP`, or as `RESOURCE_REJECT` when a registered resource cap caused the
stop. No partial one-curve or truncated-orbit bank is eligible.
...
factor. A nonunit with gcd `N` makes the complete bank a registered
`BANK_SKIP`; earlier rows are not decoded as a partial bank.
...
For a chord-supported final opaque block `b` incident to two same-curve rows,
the implementation records the public support components

\[
 b_{\rm tan}=\gcd(b,u-w),\quad
 b_{\rm chord}=\gcd(b,H(u,w)),\quad
 b_{\rm disc}=\gcd(b,\delta_E).
\]

It does not assume that either `b` or a support component is prime. When
`b_chord>1` and a third row incident to `b` exists on the same curve, it tests
`u+w+t=0 mod b_chord`. It also records the frozen scalar-index formulas. The
source tests `gcd(H(u,w),N)` separately as a direct public control. These
rational-prime channels do not determine the normalized-root sign modulo the
hidden factors of `N`.
...
The decoder emits every final opaque block, its exact sparse exponent vector,
its exact-square bit, and the factorization status `UNKNOWN_NOT_NEEDED`. This
label is never converted into an independence or primality assertion.

For a chord-support pair, scalar indices are canonicalized as `i<=j`; the
third incident index is `k`. The recorded predicates are `i+j=k`, `j-i=k`,
`2i=j`, `3i=j`, `i*j=k`, and `v2(i)=v2(j)`. These are finite pattern features
only.
