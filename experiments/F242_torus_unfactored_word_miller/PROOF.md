# Proof of the F242 quadratic-torus unfactored-word theorem
...
## 7. Four orientations, ordinary P205, and P158
...
On the P158/F172 family,

\[
 \gcd(p-1,q-1)=6
\]

and each of the other three shifted gcds is at most four.  In particular,
every `d_epsilon<=6`.  With `W=1`,

\[
 r_{p,\epsilon}\ge{p-1\over6},\qquad
 r_{q,\epsilon}\ge{q-1\over6}.
\]

Formula (6) therefore gives

\[
 S_\epsilon\le\alpha_p+\alpha_q
 \le {6\over p-1}+{6\over q-1}=2^{-\Theta(\log N)}.  \tag{P9}
\]

The random gcd exits do not change this scale.  A random `D` is a proper
zero divisor with probability at most `1/p+1/q`.  For fixed unit `D`, a
local algebra element is a nonunit with probability at most `2/r`; hence a
coefficient pair has a proper norm gcd with probability at most
`2/p+2/q` per attempt.  The acceptance probabilities from Section 4 are
constant, so the probability of seeing such a factor before acceptance is
still `O(1/p)`.  Since P158 has `p=2^{Theta(log N)}`, a numerical-
quasipolynomial number of bare `W=1` trials remains insufficient.

This is not an obstruction to an effective `W`.  P158 bounds the common
shifted part `d`; it does not bound `gcd(W,s_i)`.  Also, although a fixed
`W` sees only four residual laws, a rule producing materially different
words from the full discriminant values would be a new public integer
source.  Nothing here proves or refutes inverse-quasipolynomial progress for
such a source.
