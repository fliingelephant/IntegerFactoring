# F268-D04 independent hostile pre-run audit request
...
Authenticate `PROVENANCE.md`. Independently authenticate the immutable D03
manifest root
`19e856152caebdddc374c72444acc5f1bbd727a9c722e0e183ee2f6c70cf720f`,
its hostile `PASS` hash
`23a3e7921fa54ee1c7048ed7fc05b116f61ee76b88af2fb2275d7b13f8bd3a04`,
its failed-validation `RESULT.md` hash
`399fcea77a78c26cbfdab214dc00d186474a3ed898acb3ff0ddc2ebf16e6e459`,
and its failed-run `RUN_MANIFEST.md` hash
`f493a1302f44a9d3b3798156813bde2c3a88e7a1ba5225b3246c841974d295e9`.
Reproduce the D03 SHA-256 failure and stopped chronology. Compare D04 against
D03 and reject any change beyond `0x2748774U` to `0x2748774cU`, moving the
existing `sha256_bytes("abc")` known-answer test before family-file
authentication, packet/output version changes, and frozen
provenance/status/audit/manifest metadata.
...
12. Check that SHA-256 round constant 46 is exactly the standard
    `0x2748774cU`, with no remaining `0x2748774U` token, and that the existing
    `sha256_bytes("abc")` known-answer test executes before the first family-
    file authentication in the search self-test. Inspect the remaining C++
    self-tests for both frozen row tiers, unique `(base,exponent)` rows, a
    useful support-two certificate excluded from the decoy quotient and
    preserved across a later resource rejection, a global singleton admitted
    to it, a clean support-three quotient relation, carry algebra, P66
    decoding, and selection re-ranking.
