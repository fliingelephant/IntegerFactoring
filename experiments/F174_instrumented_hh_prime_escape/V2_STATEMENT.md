# F174 V2 candidate — an instrumented large-order search gives an exact state or a polylog-small hard prime
...
quotient capacity in every hidden prime-power component, but this procedure
certifies no exact common quotient order. F174 does not claim that this hard
block factors the input.
...
## 5. The escaped integer is prime and outside at least one old local subgroup
...
outside at least one current local cyclic layer. Only after the absolute
screen reaches its gcd-one branch is \(\beta\) known to be outside every
\(H_j\).
...
but it supplies no certified exact common quotient order.
...
- Large absolute primary order or large quotient order yields a useful
  common order.
- A QP equality bank detects a hidden local-order mismatch when one exists.
...
The surviving source question is now narrower: detect a hidden-order
mismatch when one exists (as it must on the F172 family), certify and use a
common hard order without already knowing it, or make the retained integer
presentation of one algorithm-selected prime
\(\beta=(\log n)^{O(1)}\) close a useful exact relation.
