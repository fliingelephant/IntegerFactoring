...
def perfect_powers_below(limit):
    values = set()
    for base in range(2, math.isqrt(limit - 1) + 1):
        value = base * base
        while value < limit:
            values.add(value)
            value *= base
...
def main():
    ...
    perfect_powers = perfect_powers_below(max(value[0] for value in moduli))
    counts = {
        "moduli": 0,
        "bases": 0,
        "equal_order_bases": 0,
        ...
        for base in range(2, min(BASE_LIMIT, modulus - 1) + 1):
            ...
            for old_exponent in range(2, order):
                ...
                if min(old_endpoints) <= 1 or len(set(old_endpoints)) != 4:
                    continue
                if any(value in perfect_powers for value in old_endpoints):
                    continue
                ...
                for feedback_exponent in range(1, order):
                    ...
                    if (
                        ...
                        or selected == inverse
                        or selected in perfect_powers
                        or inverse in perfect_powers
                    ):
                        continue
                    if (
                    ...
                    for old_index, old_value in enumerate(old_endpoints):
                        for new_index, new_value in enumerate(appended):
                            ...
                            witness = {
                                ...
                                "perfect_power_endpoints_excluded": True,
                            }
                            break
    ...
    output_path = Path(__file__).resolve().parents[1] / "output" / "F82-D02.json"
