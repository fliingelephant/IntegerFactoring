...
15. The four-orientation average assumes one fixed word.  A word depending
    ...
    problem.
16. No external family, theorem, asymptotic construction, or imported size
    relation is used in the V2 statement.
17. The theorem remains a conditional reduction.  It supplies no all-input
    quasipolynomial word source.

## V1 repair

V1's core theorem passed hostile audit.  Its strict statement-only
reconstruction failed only because the final bare-word warning cited the
named P158/F172 family without defining that family or proving its size
relation.  V2 deletes that imported example.  It does not change the exact
sampler, shifted-gcd identity, Miller law, four-orientation average, or bit
complexity claim.

## Status

Self-audited only.  A fresh V2 hostile audit and a fresh strict
