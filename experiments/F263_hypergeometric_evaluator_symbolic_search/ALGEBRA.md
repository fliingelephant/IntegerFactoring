# F263 algebra — block transfers for the beta-two binomial gate
...
`(alpha,beta)=(4,2)`, the central denominator `(1,1)`, and the two shifted
factors `(1,N)` and `(1,N-B)`.  This is again a control.  It moves a block
...
recurrences in the dyadic-length sequence `P(l,2^j)`.  A candidate is kept
only if it survives disjoint exact small-integer tests and two independent
public-prime modular tests.  An identity at finitely many indices is not an
...
Only classes 1 and 2 are operational candidates.  A mined identity is a
candidate shortcut only when it solves for a parent target with unit
coefficient from descriptor or already operational states and its complete
dependency DAG has numerical-quasipolynomial size.  A merge identity whose
children recursively expand to all leaves is never reclassified as a
shortcut.

The full product tree and a binary search through its singular child are
oracle controls.  Precomputing the tree costs `Theta(H)` factor evaluations
and `Theta(H)` state words.  Recomputing only the queried child at each level
also has geometric total `Theta(H)` leaf work.  `O(log H)` gcd queries do not
change either bound.

## 6. Evidence boundary

The hidden factors are never inputs to a syntax, block, projection, identity,
recurrence, canonicalization, or candidate order.  Labelled factors are used
only after public evaluation to verify a gcd, identify the true singular
index, measure query coverage, and name hostile common-capacity cohorts.
