...
Every integer \(N\ge 2\) can be factored completely by a classical Las Vegas algorithm in expected time \(\operatorname{poly}(\log N)\), without promises such as semiprimality, balanced factors, squarefreeness, special factor congruences, or smoothness conditions.
...
- A Las Vegas algorithm returns only correct outputs, terminates with probability one, and has expected running time polynomial in \(n\) for every input.
- A deterministic worst-case polynomial-time algorithm qualifies.
- A bounded-error routine qualifies only after efficient verification and an explicit Las Vegas conversion with a proved expected polynomial bound.
...
- The proof must exhibit a fixed polynomial \(P\), independent of \(N\) and its unknown factors, such that \(\mathbb E[T(N)]\le P(n)\) for every \(N\).
...
- running time polynomial in \(N\), quasipolynomial, or subexponential in \(n\);
