# F227 V4 candidate — AP exponent sampling, with an explicit all-input oracle boundary

## Status and repair boundary

This is a self-audited proof-only V4 candidate. V1, V2, V3, and their
three hostile FAILs are preserved unchanged.
...
distinct odd balanced semiprime. V4 therefore makes the child factorer an
explicit all-input oracle. It states no self-recursive factoring recurrence
and no complete numerical-QP factoring algorithm.
...
the declared per-trial screens has total useful probability

\[
\boxed{Q(n)2^{-c_0n}=2^{-\Omega(n)}.}
\tag{B6}
\]
