# F180 candidate — primary-period filtering closes synchronized two-shift extinction

## Status and scope

This is a proof-only constructive postprocessor for the F178/P160 hard
branch. It applies to arbitrary odd composites. It strengthens F179 by
processing every prime-power component of each hidden local order separately.
...
for every hidden component, and \(\sigma(f)\) denotes the largest
prime-power divisor of \(f\). Choose
...
component retains at least one primary part satisfying the condition under
the product in (7).
