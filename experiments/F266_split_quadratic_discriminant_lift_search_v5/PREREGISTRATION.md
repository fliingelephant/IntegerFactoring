# F266-D04 preregistration — split-quadratic discriminant-lift search

F266-D04 is the narrow successor to immutable failed packet F266-D03. D01,
D02, D03, and all three hostile FAIL audits remain unchanged. The algebraic
grammar, family IDs, word source, matrix caps, lift levels, tickets, decoys,
cohorts, and registered ranking tuple are unchanged. D04 keeps every valid
D03 repair. It changes only the convenience-sidecar binding and the exact
source/base stage definition exposed by the direct counters.
...
The mandatory self-test fixes the eight schema names in this order. It sends
four unit candidates only through `source`, sends exactly
`A0,B0,C0,Delta0,x` only through `base`, and requires all six later counters
to remain zero.
...
It then hashes the selection and corpus bytes independently. For each
convenience sidecar, it requires exactly one newline-terminated record, the
captured digest in the digest field, two ASCII spaces, and the fixed basename
of its target in the filename field. Extra bytes, alternate filenames, and
alternate digests are fatal. It repeats these exact sidecar checks after
heldout and before the complete uncompressed manifest. The sidecars are never
a digest authority. Heldout receives the captured selection digest as a
command argument, reads the selection bytes once, verifies the digest
internally, parses those same bytes, and requires four unique known IDs. The
parser requires those IDs, in order, to equal the first four IDs in the
authenticated rank rows and validates every rank-field syntax. It reads no
discovery certificate or metric file.

Before compilation, a focused runner self-check accepts the one canonical
sidecar record and rejects a wrong filename, wrong digest, and wrong
separator. Discovery and heldout bank files must also expose exactly the 32
registered direct-counter columns in stage/outcome order, and every row must
satisfy all eight partition identities.
...
The F266-D01, F266-D02, and F266-D03 FAIL audits remain attached only to their
exact versions. None authorizes a D04 step. D04 needs a fresh independent
audit of its own frozen bytes.
