...
bool record_ticket(BankResult& result, const cpp_int& candidate, const cpp_int& N,
  ...
  bool useful = record_factor(result,candidate,N,channel,carry);
  if (useful && first) {
...
bool screened_unit(const cpp_int& value, const cpp_int& N, BankResult& result,
  ...
  if (g == 1) return true;
  if (g < N) record_ticket(result,value,N,channel);
  return false;
}
...
void screen_base_channels(const cpp_int& N, BankResult& result) {
  ...
  for (const Row& row:result.rows) {
    ...
    for (const auto& item : std::array<std::pair<cpp_int,std::string>,5>{{
      ...
      record_ticket(result,item.first,N,item.second,{row});
  }
}
...
void screen_projective_channels(const cpp_int& N, BankResult& result) {
  for (const Row& row:result.rows) {
    ...
    record_ticket(result,determinant,N,"PROJECTIVE_ROOT_DIFFERENCE",{row});
  }
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      ...
      for (const auto& a:xr) for (const auto& b:yr) {
        ...
        ++result.root_determinants;
        record_ticket(result,cross,N,"CROSS_PROJECTIVE_ROOT",{x,y});
      }
    }
  }
}
...
void screen_resultants(const cpp_int& N, BankResult& result) {
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      ...
      ++result.resultants;
      record_ticket(result,res,N,"QUADRATIC_RESULTANT",{x,y});
    }
  }
}
...
void screen_carries(const cpp_int& N, BankResult& result) {
  for (const Row& row:result.rows) {
    ...
    record_ticket(result,row.carry,N,"INVARIANT_CARRY",{row},
                  row.value-row.delta0,row.modulus,true);
  ...
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      ...
      record_ticket(result,quotient,N,x.level==y.level ?
                    ...
                    {x,y},x.value-y.value,divisor,true);
      if (!before&&result.any_factor) {
        result.strict_carry=true;
      }
    }
...
void screen_singletons_and_square_classes(const cpp_int& N, BankResult& result) {
  for (const Row& row:result.rows) {
    ...
    if (cls=="USEFUL") {
      ...
      bool first=result.first_factor==1;
      record_factor(result,exact-row.root,N,label+"_MINUS");
      record_factor(result,exact+row.root,N,label+"_PLUS");
  ...
  for (std::size_t i=0;i<result.rows.size();++i) {
    for (std::size_t j=i+1;j<result.rows.size();++j) {
      ...
      if (cls=="USEFUL") {
        ...
        bool first=result.first_factor==1;
        record_factor(result,exact-supplied,N,label+"_MINUS");
        record_factor(result,exact+supplied,N,label+"_PLUS");
        EvidenceCertificate certificate;
        certificate.present=true;certificate.kind="NORMALIZED_ROOT";certificate.label=label;
        certificate.root_class=cls;certificate.exact_root=exact;
...
std::string corpus_bytes(const std::vector<Case>& cases) {
  ...
  for (const Case& c:cases)
    out<<"F266-D02\t"<<c.split<<'\t'<<c.shape<<'\t'<<c.factor_bits<<'\t'
       <<c.index<<'\t'<<c.N<<'\n';
  return out.str();
}
...
void emit_certificate_json(std::ostringstream& out,const Case& c,int family,
  ...
  out<<"{\"version\":\"F266-D02\",\"split\":"<<json_string(c.split)
...
std::string opaque_bytes(const std::vector<Case>& cases,
  ...
  for (const Evaluated& evaluated:rows) {
    const Case& c=cases[evaluated.case_index];const BankResult& b=evaluated.bank;
    if (b.eligible) {
      out<<"{\"version\":\"F266-D02\",\"kind\":\"ROW_MAP\",\"split\":"
    ...
    for (std::size_t block_index=0;block_index<b.opaque_blocks.size();++block_index) {
      ...
      out<<"{\"version\":\"F266-D02\",\"kind\":\"OPAQUE_BLOCK\",\"split\":"
         <<json_string(c.split)
         <<",\"shape\":"<<json_string(c.shape)<<",\"factor_bits\":"<<c.factor_bits
         <<",\"index\":"<<c.index<<",\"N\":"<<json_string(c.N.convert_to<std::string>())
...
Selection parse_selection(const std::string& bytes,const std::string& expected_digest) {
  ...
  require(bool(std::getline(input,line))&&line=="F266-D02-selection-v1","selection header");
...
std::string lead_gate_bytes(const std::vector<Case>& cases,
  ...
  for (const Evaluated& evaluated:rows) {
    ...
    if (b.eligible) {
      ...
      ++base.banks;base.base+=b.base_singletons;base.orbit+=b.orbit_singletons;
  ...
  out<<"cell\tfactor_bits\tshape\teligible_banks\tobserved_base_singletons"
...
std::set<std::string> parse_discovery_corpus(const std::string& bytes) {
  ...
  while (std::getline(input,line)) {
    ...
    std::istringstream row(line);std::vector<std::string> fields;std::string field;
    while (std::getline(row,field,'\t')) fields.push_back(field);
    require(fields.size()==6&&fields[0]=="F266-D02"&&fields[1]=="discovery",
...
void run_discovery(const std::string& directory,int workers) {
  ...
  const std::string corpus_name="F266-D02.discovery.corpus.tsv";
  const std::string corpus_sha_name="F266-D02.discovery.corpus.sha256";
  const std::string bank_name="F266-D02.discovery.banks.tsv";
  const std::string family_name="F266-D02.discovery.families.tsv";
  const std::string certificate_name="F266-D02.discovery.certificates.jsonl";
  const std::string opaque_name="F266-D02.discovery.opaque.jsonl";
  const std::string selection_name="F266-D02.selection.tsv";
  const std::string selection_sha_name="F266-D02.selection.sha256";
  ...
  write_text(join_path(directory,"F266-D02.discovery.manifest.tsv"),manifest_bytes(directory,names));
...
void run_heldout(const std::string& directory,const std::string& selection_path,
  ...
  auto start=std::chrono::steady_clock::now();
  for (const char* name:{"F266-D02.discovery.corpus.tsv","F266-D02.discovery.corpus.sha256",
                         "F266-D02.discovery.banks.tsv","F266-D02.discovery.families.tsv",
                         "F266-D02.discovery.certificates.jsonl","F266-D02.discovery.opaque.jsonl",
                         "F266-D02.selection.tsv",
                         "F266-D02.selection.sha256","F266-D02.discovery.manifest.tsv"})
  ...
  const std::string corpus_name="F266-D02.heldout.corpus.tsv";
  const std::string bank_name="F266-D02.heldout.banks.tsv";
  const std::string family_name="F266-D02.heldout.families.tsv";
  const std::string certificate_name="F266-D02.heldout.certificates.jsonl";
  const std::string opaque_name="F266-D02.heldout.opaque.jsonl";
  const std::string lead_name="F266-D02.heldout.lead_gate.tsv";
  write_text(join_path(directory,corpus_name),corpus);
  write_text(join_path(directory,bank_name),banks);
  write_text(join_path(directory,family_name),families);
  write_text(join_path(directory,certificate_name),certificates);
  ...
  write_text(join_path(directory,"F266-D02.heldout.manifest.tsv"),manifest_bytes(directory,names));
...
void run_preflight(const std::string& output_path,int workers) {
  ...
  u64 sample_output=corpus_bytes(cases).size()+sample_banks.size()+sample_families.size()+
  ...
  json<<"{\"version\":\"F266-D02\",\"pass\":"<<(pass?"true":"false")
