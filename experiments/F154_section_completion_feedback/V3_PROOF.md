# Proof of the F154 V3 section-completion theorem
...
## 7. The \(N=77\) refinement, subgroup, and later-power certificate
...
\(30\). Since \(26^{-1}\equiv3\) and \(9\equiv3^2\equiv26^{-2}\),
...
growth of the group of all publicly computable residues:
\(26=\iota_{77}(3)\) is already the public inverse of the supplied root
\(3\).

The current completion decoder and the displayed endpoint gcd screens do
not factor \(77\). Section 3 gives normalized root \(+1\) for every
completion dependency, while
...
The nontrivial gcd \(\gcd(26,4706)=26\) factors the old endpoint, not \(N\).

The refined name does support a later operation that factors this finite
input. Since \(26\equiv5\pmod7\) and \(26\equiv4\pmod{11}\),

\[
26^{15}\equiv-1\pmod7,
\qquad
26^{15}\equiv1\pmod{11}.
\]

The simultaneous residue is

\[
26^{15}\equiv34\pmod{77}.
\]

It is a non-global square root of \(1\), and the standard gcd screens give
...
Therefore a later grammar that admits the newly named block \(26\) through
the restricted interface and performs the exponent-\(15\) power test
factors \(77\). This is a finite refinement-mediated capability witness. It
is not new modular information: \(26=\iota_{77}(3)\) was publicly computable
before completion, and the same modular test could be expressed using that
old public residue in an unrestricted grammar.

## 8. No all-input success law

The finite power test does not supply an all-input rule for choosing the
exponent \(15\), finding a useful order, or obtaining a non-global
half-power. The theorem gives no lower bound on refinement, no descent or
termination potential, and no reason that a later canonical relation must
disagree with the section on general inputs.
...
The claimed gain is therefore one finite and conditional,
refinement-mediated grammar capability together with a general
decoder-inert completion theorem. F154 proves no forced refinement, later
disagreement, iteration bound, all-input order-selection law, or all-input
factoring algorithm.
