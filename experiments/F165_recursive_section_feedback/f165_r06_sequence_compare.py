#!/usr/bin/env python3
"""Registered semantic comparison of F165-D01 and F165-R06 sequences.

This source does not import or execute either F165 implementation. It reads
their frozen outputs, translates the richer R06 transcript into the sequence
...
from the frozen R06 transcript.
"""
...
def main() -> int:
    ...
    expected_outputs = json.loads(
        (directory / "F165_R06_EXPECTED_OUTPUTS.json").read_text()
    )
    prior_r04 = json.loads(
        (directory / "f165_r04_v2_blind_output.json").read_text()
    )
    r04 = json.loads(
        (directory / "f165_r06_v2_reconstruct_output.json").read_text()
    )
    audit = Audit()
    ...
    expected_reconstruction = expected_outputs["reconstruction"]
    audit.equal(
        "expected_output",
        "R06.schema",
        expected_reconstruction["schema"],
        r04.get("schema"),
    )
    audit.equal(
        "expected_output",
        "R06.final_verdict",
        expected_reconstruction["final_verdict"],
        r04.get("final_verdict"),
    )
    audit.equal(
        "expected_output",
        "R06.aggregate_sequence_hashes",
        expected_reconstruction["aggregate_sequence_hashes"],
        r04.get("aggregate_sequence_hashes"),
    )
    audit.equal(
        "expected_output",
        "R06.aggregate_value_only_hashes",
        expected_reconstruction["aggregate_value_only_hashes"],
        r04.get("aggregate_value_only_hashes"),
    )
    audit.equal(
        "expected_output",
        "R06.aggregates",
        expected_reconstruction["aggregates"],
        r04.get("aggregates"),
    )
    audit.equal(
        "corpus",
    ...
    prior_r04_instances = prior_r04["results"]
    audit.equal("corpus", "instance_count", len(d01_instances), len(r04_instances))
    audit.equal(
        "r06_only_change",
        "prior_R04.instance_count",
        len(prior_r04_instances),
        len(r04_instances),
    )
    ...
    for corpus_index, (d01_entry, r04_entry) in enumerate(
        ...
        prior_r04_entry = prior_r04_instances[corpus_index]
        ...
        audit.equal("corpus", f"{prefix}.R06.N", modulus, r04_entry["N"])
        ...
        for stage_index in range(3):
            ...
            prior_r04_summary = (
                prior_r04_entry["base"]
                if stage_name == "base"
                else prior_r04_entry["levels"][stage_index - 1]
            )
            ...
            for name in SEQUENCE_NAMES:
                audit.equal(
                    "r06_only_change",
                    f"{stage_path}.sequence_unchanged_from_R04.{name}",
                    prior_r04_summary["snapshot"]["sequences"][name],
                    sequences[name],
                )
                audit.equal(
                    "r06_hash_integrity",
            ...
            audit.equal(
                "decoder_summary",
                f"{stage_path}.R06_retained_columns",
                retained_columns,
                r04_summary["columns"],
            )
            audit.equal(
            ...
            if stage_name == "base":
                correction_counts["base_snapshots_unchanged"] += 1
                for field in ("rows", "columns", "rank", "nullity"):
                    audit.equal(
                        "r06_only_change",
                        f"{stage_path}.{field}_unchanged_from_R04",
                        prior_r04_summary[field],
                        r04_summary[field],
                    )
                audit.equal(
                    "r06_only_change",
                    f"{stage_path}.root_status_unchanged_from_R04",
                    prior_r04_summary["normalized_root_status"],
                    r04_summary["normalized_root_status"],
                )
            else:
                has_zero_column = any(record["A"] == 1 for record in ledger)
                correction_counts[
                    "feedback_snapshots_with_retained_A_equal_1_zero_column"
                ] += int(has_zero_column)
                audit.equal(
                    "r06_only_change",
                    f"{stage_path}.retains_A_equal_1",
                    True,
                    has_zero_column,
                )
                for field in ("rows", "rank"):
                    audit.equal(
                        "r06_only_change",
                        f"{stage_path}.{field}_unchanged_from_R04",
                        prior_r04_summary[field],
                ...
                for field in ("columns", "nullity"):
                    audit.equal(
                        "r06_only_change",
                        f"{stage_path}.{field}_delta_from_R04",
                        prior_r04_summary[field] + 1,
                        r04_summary[field],
                    )
                for status in ("global_minus", "non_global"):
                    audit.equal(
                        "r06_only_change",
                        f"{stage_path}.{status}_unchanged_from_R04",
                        prior_r04_summary["normalized_root_status"][status],
                        r04_summary["normalized_root_status"][status],
                    )
                audit.equal(
                    "r06_only_change",
                    f"{stage_path}.global_plus_delta_from_R04",
                    prior_r04_summary["normalized_root_status"]["global_plus"] + 1,
                    r04_summary["normalized_root_status"]["global_plus"],
                )
    ...
    expected_comparison = expected_outputs["comparison"]
    audit.equal(
        "expected_output",
        "comparison.instances_compared",
        expected_comparison["instances_compared"],
        min(len(d01_instances), len(r04_instances)),
    )
    audit.equal(
        "expected_output",
        "comparison.stages_compared",
        expected_comparison["stages_compared"],
        len(r04_stages),
    )
    audit.equal(
        "expected_output",
        "comparison.comparison_counts",
        expected_comparison["comparison_counts"],
        dict(comparison_counts),
    )
    audit.equal(
        "expected_output",
        "comparison.normalized_sequence_sha256",
        expected_comparison["normalized_sequence_sha256"],
        normalized_hashes,
    )
    expected_correction = expected_reconstruction["decoder_correction"]
    audit.equal(
        "expected_output",
        "decoder_correction.base_snapshots_unchanged",
        expected_correction["base_snapshots_unchanged"],
        correction_counts["base_snapshots_unchanged"],
    )
    audit.equal(
        "expected_output",
        "decoder_correction.feedback_A_equal_1_snapshots",
        expected_correction[
            "feedback_snapshots_with_retained_A_equal_1_zero_column"
        ],
        correction_counts[
    ...
    payload = {
        ...
        "claim_scope": (
            "semantic equality of the complete ordered D01/R06 candidate, "
        ...
        "normalized_sequence_sha256": normalized_hashes,
        "decoder_correction_counts": dict(correction_counts),
