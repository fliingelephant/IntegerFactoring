...
std::string shortfall_bytes(
  ...
  for (const auto& [split, bits, missing] : shortfalls)
    out << "F268-D03\t" << split << "\tmarker-control\t" << bits << '\t'
        << missing << '\n';
  return out.str();
}
...
void generate(const std::string& directory) {
  ...
  write_new(join_path(directory, "F268-D03.discovery.public.tsv"),
            discovery_public, 0644);
  write_new(join_path(directory, "F268-D03.heldout.public.tsv"),
            heldout_public, 0644);
  write_new(join_path(directory, "F268-D03.discovery.labels.tsv"),
            discovery_labels, 0600);
  write_new(join_path(directory, "F268-D03.heldout.labels.tsv"),
            heldout_labels, 0600);
  write_new(join_path(directory, "F268-D03.marker_shortfalls.tsv"),
