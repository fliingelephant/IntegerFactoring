# F144 V2 candidate statement — positive cross-star cycles require square-root-scale anchor mass

## Status and scope

This is the corrected proof-only boundary for the P128 lifted source.  The
frozen first statement and proof are preserved unchanged.  Their hostile
audit found that the proposed short-cycle surplus regime was vacuous.
V2 removes that regime and promotes its exact cause to the main theorem.
...
2. Every positive-direction cycle containing a canonical wrap already needs
   ...
   square.  Thus bounded positive cycles with total raw anchor product
...
V2 concerns stored positive-direction containment cycles.  It permits a
...
\[
\boxed{\mathcal A>\sqrt{N+S^2}>\sqrt N.}
\tag{15}
\]
...
but V2 gives no automatic root-asymmetry theorem for them.
...
The interval in (15)--(16) is real.  It is
...
The exact \(N=35\) certificate in Section 6 lies in this interval.
...
This is an arithmetic impossibility, not a runtime lower bound.
...
quasipolynomial.  V2 therefore rules out the bounded-magnitude positive-cycle
idea, not the full P128 lifted source.
...
determinant and P115--P116 boundaries apply without change.  V2 makes no
density claim for that scalar.

## 8. Exact remaining gate

The positive-direction target is no longer a bounded short cycle.  Any
...
V2 proves no all-input existence theorem and no factoring algorithm.
