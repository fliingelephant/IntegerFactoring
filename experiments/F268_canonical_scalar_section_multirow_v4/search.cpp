...
std::vector<Case> parse_public_corpus(const std::string& path,
  ...
  while (std::getline(input, line)) {
    ...
    require(f.size() == 6 && f[0] == "F268-D04" && f[1] == expected_split,
            "public corpus row");
...
std::string bank_bytes(const std::vector<Case>& cases,
  ...
  for (const Evaluated& item:evaluated) {
    ...
    out<<"F268-D04\t"<<c.split<<'\t'<<c.shape<<'\t'<<c.factor_bits<<'\t'
...
Selection parse_selection(const std::string& bytes,const std::string& digest) {
  ...
  require(bool(std::getline(input,line))&&line=="F268-D04-selection-v1",
          "selection header");
...
void run_preflight(const std::string& discovery_path,const std::string& discovery_sha,
  ...
  write_new(join_path(directory,"F268-D04.preflight.banks.tsv"),banks);
  write_new(join_path(directory,"F268-D04.preflight.evidence.tsv"),evidence);
  write_new(join_path(directory,"F268-D04.preflight.metrics.tsv"),
...
void run_discovery(const std::string& corpus_path,const std::string& corpus_sha,
  ...
  make_directory(directory);
  write_new(join_path(directory,"F268-D04.discovery.banks.tsv"),banks);
  write_new(join_path(directory,"F268-D04.discovery.evidence.tsv"),evidence);
  write_new(join_path(directory,"F268-D04.discovery.stats.tsv"),stats);
  write_new(join_path(directory,"F268-D04.selection.tsv"),selection);
  write_new(join_path(directory,"F268-D04.selection.sha256"),
            selection_sha+"  F268-D04.selection.tsv\n");
  ResourceSnapshot resource=resources(start);
  ...
  write_new(join_path(directory,"F268-D04.discovery.metrics.tsv"),metrics.str());
...
void run_heldout(const std::string& corpus_path,const std::string& corpus_sha,
  ...
  make_directory(directory);
  write_new(join_path(directory,"F268-D04.heldout.banks.tsv"),banks);
  write_new(join_path(directory,"F268-D04.heldout.evidence.tsv"),evidence);
  write_new(join_path(directory,"F268-D04.heldout.stats.tsv"),stats);
  write_new(join_path(directory,"F268-D04.heldout.lead.tsv"),lead);
  ResourceSnapshot resource=resources(start);
  std::ostringstream metrics;
  metrics<<std::fixed<<std::setprecision(6)<<"wall_seconds\t"<<resource.wall
         <<"\nuser_seconds\t"<<resource.user<<"\nsystem_seconds\t"<<resource.system
         <<"\nmaxrss_kib\t"<<resource.rss<<"\noutput_bytes\t"<<total<<'\n';
  write_new(join_path(directory,"F268-D04.heldout.metrics.tsv"),metrics.str());
