# F268-D02 independent hostile pre-run audit request
...
Authenticate `PROVENANCE.md`. Independently authenticate the immutable D01
manifest root
`b99d47d7ba7a01c456a0b755a5e77f42a237be73ffd6ea422e38d03b076a1cc9`
and its post-freeze FAIL record hash
`8ab91efa95e1aa20f0985f80c28790340987d4b61f3bbb662707587a4b2642c2`.
Confirm that D02 directly includes `<tuple>` before using `std::tie`, that no
D01 dynamic output exists, and that the remaining change is the packet/output
version token plus frozen provenance.
...
9. Check the random, nonconsecutive-neighbor, safe-safe, and bounded
   ...
   explicit marker shortfalls, and post-run label verification.
10. Audit all caps and resource dispositions. In particular check source
    ...
    A cap crossing must be `RESOURCE_REJECT`, never a null bank.
