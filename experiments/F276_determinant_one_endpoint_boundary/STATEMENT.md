# F276 statement — determinant-one endpoint laws and affine transfer boundary

## Status and scope

This is a proof-only candidate. It contains no computation, search result, or
factoring algorithm.
...
The two positive controls fail `UNIT` or `STATE/ENDPOINT`. The easy affine
branch fails `SIGNAL`. The generic affine branch has no `ENDPOINT` or
`SIGNAL` theorem.

## 4. Search decision and exact exclusions

No C++ search is justified for rational two-endpoint laws, separable length
laws, unimodular direct-summand multipliers, whole-state coordinate gcds, or
the reducible affine branch. They can only rediscover a gauge telescope, a
