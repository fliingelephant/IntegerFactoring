...
work_dir=/root/IntegerFactoring_F266/F266-D03
...
refuse_overlap() {
  ...
  for proc in /proc/[0-9]*; do
    ...
    if [[ $cmd =~ search|symbolic_search|remote_run|validate ]]; then
      echo "F266-D03 refuses overlap with pid=$pid cwd=$cwd cmd=$cmd" >&2
      exit 73
    fi
  done
}
...
for path in "$out_dir" "$log_dir" "$preflight_dir" "$archive_dir" "$work_dir/search"; do
  if [[ -e $path ]]; then
    echo "F266-D03 refuses to overwrite $path" >&2
    exit 74
  fi
done
...
(
  ...
  timeout "${remaining}s" nice -n 15 ./search --preflight \
    "$preflight_dir/F266-D03.preflight.json" 1
) >"$log_dir/preflight.stdout" 2>"$log_dir/preflight.stderr"
preflight_files=$(find "$preflight_dir" -maxdepth 1 -type f -printf '%f\n' | sort)
[[ $preflight_files == 'F266-D03.preflight.json' ]]
grep -q '"pass":true' "$preflight_dir/F266-D03.preflight.json"
write_manifest "$log_dir/preflight.manifest.tsv" \
  "$preflight_dir/F266-D03.preflight.json" \
...
write_manifest "$log_dir/discovery.manifest.tsv" \
  ...
  "$out_dir/F266-D03.discovery.manifest.tsv" \
  "$out_dir/F266-D03.selection.tsv" "$out_dir/F266-D03.selection.sha256"
check_uncompressed_cap
...
} >"$log_dir/F266-D03.run_manifest.tsv"
...
mapfile -t evidence_files < <(
  {
    ...
    find output logs preflight -type f ! -name 'F266-D03.uncompressed.manifest.tsv' \
      ! -name 'F266-D03.uncompressed.manifest.tsv.sha256' -print
  } | sort
)
write_manifest "$log_dir/F266-D03.uncompressed.manifest.tsv" "${evidence_files[@]}"
timeout "$(remaining_seconds)s" sha256sum "$log_dir/F266-D03.uncompressed.manifest.tsv" \
  >"$log_dir/F266-D03.uncompressed.manifest.tsv.sha256"
check_uncompressed_cap
uncompressed_bytes=$(
  {
    du -sb "$out_dir" "$preflight_dir" "$log_dir" | awk '{print $1}'
...
(
  ...
  timeout "${remaining}s" nice -n 15 bash -o pipefail -c '
    tar -cf - \
      ...
      | zstd -q -T1 -3 -o "$1"
  ' F266-D03-compression "$archive_dir/F266-D03.evidence.tar.zst"
)
archive_bytes=$(stat -c '%s' "$archive_dir/F266-D03.evidence.tar.zst")
[[ $archive_bytes -le 1073741824 ]]
remaining=$(remaining_seconds)
timeout "${remaining}s" sha256sum "$archive_dir/F266-D03.evidence.tar.zst" \
  >"$archive_dir/F266-D03.evidence.tar.zst.sha256"
write_manifest "$archive_dir/F266-D03.compression.manifest.tsv" \
  "$archive_dir/F266-D03.evidence.tar.zst" \
  "$archive_dir/F266-D03.evidence.tar.zst.sha256"
timeout "$(remaining_seconds)s" sha256sum \
  "$archive_dir/F266-D03.compression.manifest.tsv" \
  >"$archive_dir/F266-D03.compression.manifest.tsv.sha256"
archive_total_bytes=$(du -sb "$archive_dir" | awk '{print $1}')
[[ $archive_total_bytes -le 1073741824 ]]

echo "F266-D03 complete selection_sha256=$selection_sha corpus_sha256=$corpus_sha uncompressed_bytes=$uncompressed_bytes archive_bytes=$archive_bytes archive_total_bytes=$archive_total_bytes"
