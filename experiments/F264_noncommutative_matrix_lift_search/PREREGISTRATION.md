# F264-D01 preregistration — noncommutative matrix-word lift search

## Status and purpose

This packet freezes a finite C++17 discovery experiment. It searches public
...
every F258--F263 production or validation run.
...
For each public odd `N`, derive two profiles from the 128-bit public hash of
the complete integer `N` and the constants in `symbolic_search.cpp`. In each
profile choose public nonzero residues `a,b,c,u` modulo `N`. Apply `gcd` screens
to every required denominator or unit before use.
...
identity in `ALGEBRA.md`. The executable aborts on a failed divisibility test.

To keep the full word layer resource-safe, scalar row families use the public
coordinate `q=SplitMix64(profile,word code,length) mod 4`; a two-coordinate
...
All 8,744 words contribute row atoms. The following larger scopes use only a
public SplitMix64 priority and fixed caps per profile:
...
smooth-order programs:                       16 main words + 2 controls
```

Priority inputs are only `N`, profile, word code, scope tag, and the frozen
source constant. No hidden label or previous score enters a cap.
...
10 step_cross_minors              2-minors among K,C_parent,C_word
...
29 split_root_residual            s^2-character_discriminant
30 conjugacy_coordinate_control   entry changes under public conjugation
```

The exact global zeros are counted separately and never inserted into an
integer word: full matrix associativity, trace cyclicity, inverse trace,
Fricke modulo `N^2`, Cayley--Hamilton modulo `N^2`, determinant
multiplicativity, and conjugacy invariance of trace and determinant.

## Typed synthesized grammar

For each family form its public atom sum `S_f` and atom product `P_f`, reduced
modulo the scoring modulus. The bounded expression DAG contains:
...
The engine must rediscover the frozen relations, compute their modular ranks,
and authenticate them by exact or divisibility checks on 64 disjoint public
synthetic inputs. Unexpected extra ternary null vectors abort validation.
...
features must remain unchanged. Ordered coordinate and carry features are
reported in both bases and remain distinct typed objects.
...
`N`. Zero and units are counted. Compute `gcd(atom,N)` before multiplication
into a family word. Every synthesized candidate is also gcd-tested through
its residue. Only a proper gcd is a factor ticket.
...
so the exact common matrix order divides every `r-1`. Candidate roots are
the frozen ordered entry/lift combinations in `ALGEBRA.md`; no square-root
oracle is assumed. Report the accumulated certified lcm and its new bits.
...
`s_q`; this preserves the exact gcds. Record both residual gcds and the exact
baseline improvement. No factorization of `W` is used.
...
cohorts at one size are rejected. Total planned inputs: 5,280. Cohort seeds
are frozen in `symbolic_search.cpp`. Safe-safe is the hostile cohort.

Discovery and held-out rows are written separately in the output. Since the
candidate universe is exhaustive and frozen, discovery ranking changes no
held-out candidate set or threshold.
...
- A direct family is a finite lead only with at least 16 held-out hits across
  three held-out sizes, including safe-safe.
- A common-order lead must add noncyclic certified lcm support on held-out and
  safe-safe inputs. Cyclic controls do not count.
- A P205 word is a finite lead only if its safe-safe median residual loss grows
  visibly slower than factor size at all held-out sizes and random inputs agree.
...
4-hour deadline. Sparse anomaly rows are capped; no candidate-by-atom dense
matrix is emitted. The planned hot loop is 8,744 word steps, at most 2,048
pair scopes, 512 triple scopes, 36 smooth-order trials, and about 11,000
synthesized residues per input.

The authoritative benchmark is the complete largest 60-bit-factor public
pipeline, including word enumeration, pair/triple scopes, candidate gcds,
order trials, and P205 scoring. The runner refuses production unless a
conservative 1.75x eight-thread projection is at most 14,400 seconds and the
predicted output is below 1 GiB.

The runner checks processes both before validation and before each data split.
It refuses any active `F258` through `F263` job. F264 may be compiled while an
incompatible job exists, but no self-test, benchmark, discovery, or held-out
cohort may run. This packet freezes source and workflow before those gates are
opened. No durable ledger may be edited by this experiment.
