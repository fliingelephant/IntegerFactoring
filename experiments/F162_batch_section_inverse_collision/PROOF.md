# Proof of the F162 batch signed-inverse detector
...
`O(m log m)`. Storage is soft-linear in `mn`. This proves the general batch
theorem and its factor-free cost.
...
owner lists through intersection cap `t`. Summing the soft-linear batch cost
over groups gives the stated dependence on `S_t`. For
`t=polylog(n)` and an explicit quasipolynomial transcript, the binomial
enumeration and every batch remain quasipolynomial.
