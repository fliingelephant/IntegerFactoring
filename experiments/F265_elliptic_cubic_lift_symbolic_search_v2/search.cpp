...
constexpr int RANDOM_BELOW_MAX = 4096;
constexpr uint64_t CHORD_TESTS_MAX = 200000;
constexpr size_t SAVED_PATTERNS_MAX = 64;
constexpr size_t JOB_BATCH_MAX = 64;

struct ResourceReject : std::runtime_error {
...
bool make_case(int split, int bits, int shape, int index,
               std::set<std::string>& used, Case& out, FactorLabel& label) {
...
Corpus make_corpus(int wanted_split) {
    ...
    for (int bits : bits_menu) {
      for (int shape = 0; shape < 3; ++shape) {
        for (int index = 0; index < 16; ++index) {
          Case c;
          FactorLabel label;
          if (!make_case(split, bits, shape, index, used, c, label)) {
            if (split == wanted_split) ++corpus.shortfall;
            continue;
          }
...
struct PatternRecord {
  int block_id = -1;
  cpp_int block_value, support_value, tangent_support, discriminant_support;
  cpp_int A, B, delta, first_u, second_u, third_u;
  int curve = -1;
  int first_row = -1, second_row = -1, third_row = -1;
  int i = 0, j = 0, k = 0;
  bool third_exists = false, third_root_match = false;
  bool sum = false, difference = false, twice = false, thrice = false;
  bool product = false, equal_binary_valuation = false;
};
...
struct BankResult {
  ...
  uint64_t gcd_free_splits = 0, pattern_work = 0, chord_block_pairs = 0;
  uint64_t third_root_tests = 0, third_root_matches = 0;
  uint64_t index_sum = 0, index_difference = 0, index_twice = 0;
  uint64_t index_thrice = 0, index_product = 0, index_equal_v2 = 0;
  bool residual_bank = false;
  std::vector<CurveInfo> curves;
  std::vector<Row> rows_data;
  std::vector<OpaqueBlockRecord> block_records;
  ...
  while (changed) {
    ...
    for (size_t i = 0; i < d.blocks.size() && !changed; ++i) {
      for (size_t j = i + 1; j < d.blocks.size(); ++j) {
        ...
        ++d.splits;
        if (d.splits > SPLITS_MAX || static_cast<int>(d.blocks.size()) > BLOCKS_MAX) {
          d.resource_reject = true;
...
Relation verify_relation(const std::vector<uint64_t>& bits,
  ...
  rel.gcd_minus = gm;
  rel.gcd_plus = gp;
  for (int col : rel.cols) {
    const Row& row = br.rows_data[col];
    const CurveInfo& curve = curve_by_id(br, row.curve);
    rel.rows.push_back({row.curve, row.index, curve.A, curve.B,
                        row.u, row.v, row.a, row.carry});
  }
  return rel;
}

bool mine_chord_patterns(BankResult& br, const std::vector<Block>& blocks) {
  for (int block_id = 0; block_id < static_cast<int>(blocks.size()); ++block_id) {
    const Block& block = blocks[block_id];
    std::vector<int> incident;
    for (int row = 0; row < static_cast<int>(br.rows_data.size()); ++row)
      if (block.exponents[row] != 0) incident.push_back(row);
    for (size_t aa = 0; aa < incident.size(); ++aa) {
      for (size_t bb = aa + 1; bb < incident.size(); ++bb) {
        const int first_row = incident[aa], second_row = incident[bb];
        const Row& r = br.rows_data[first_row];
        const Row& s = br.rows_data[second_row];
        if (r.curve != s.curve) continue;
        if (++br.pattern_work > CHORD_TESTS_MAX) {
          br.resource_reject = true;
          br.error = "CHORD_PATTERN_CAP";
          return false;
        }
        const CurveInfo& curve = curve_by_id(br, r.curve);
        cpp_int H = r.u * r.u + r.u * s.u + s.u * s.u + curve.A;
        cpp_int support = gcdz(block.value, H);
        if (support == 1) continue;
        ++br.chord_block_pairs;
        const int i = std::min(r.index, s.index);
        const int j = std::max(r.index, s.index);
        const bool twice = 2 * i == j;
        const bool thrice = 3 * i == j;
        const bool equal_v2 =
            __builtin_ctz(static_cast<unsigned>(i)) ==
            __builtin_ctz(static_cast<unsigned>(j));
        br.index_twice += twice;
        br.index_thrice += thrice;
        br.index_equal_v2 += equal_v2;
        bool found_third = false;
        for (int third_row : incident) {
          if (third_row == first_row || third_row == second_row) continue;
          const Row& t = br.rows_data[third_row];
          if (t.curve != r.curve) continue;
          found_third = true;
          if (++br.pattern_work > CHORD_TESTS_MAX) {
            br.resource_reject = true;
            br.error = "CHORD_PATTERN_CAP";
          ...
          rec.block_value = block.value;
          rec.support_value = support;
          rec.tangent_support = gcdz(block.value, r.u - s.u);
          rec.discriminant_support = gcdz(block.value, curve.delta);
          rec.A = curve.A;
          rec.B = curve.B;
          rec.delta = curve.delta;
          rec.first_u = r.u;
          rec.second_u = s.u;
          rec.third_u = t.u;
          rec.curve = r.curve;
          rec.first_row = first_row;
          rec.second_row = second_row;
          rec.third_row = third_row;
          rec.i = i;
          rec.j = j;
          rec.k = t.index;
          rec.third_exists = true;
          rec.third_root_match = modz(r.u + s.u + t.u, support) == 0;
          rec.sum = i + j == rec.k;
          rec.difference = j - i == rec.k;
          rec.twice = twice;
          rec.thrice = thrice;
          rec.product = i * j == rec.k;
          rec.equal_binary_valuation = equal_v2;
          br.third_root_matches += rec.third_root_match;
          br.index_sum += rec.sum;
          br.index_difference += rec.difference;
          br.index_product += rec.product;
          if (br.patterns.size() < SAVED_PATTERNS_MAX)
            br.patterns.push_back(std::move(rec));
        }
        if (!found_third && br.patterns.size() < SAVED_PATTERNS_MAX) {
          PatternRecord rec;
          rec.block_id = block_id;
          rec.block_value = block.value;
          rec.support_value = support;
...
BankResult analyze_bank(const Case& c, const Family& f) {
  ...
  try {
    ...
    for (int curve_id = 0; curve_id < 2; ++curve_id) {
      CurveInfo curve;
      if (!choose_curve(c, f, curve_id, curve, br)) {
        complete = false;
        break;
      }
      br.curves.push_back(curve);
      if (!append_curve_rows(c, f, curve, br)) {
        complete = false;
        break;
      }
    }
    br.rows = br.rows_data.size();
    if (!complete || br.curves.size() != 2) {
      if (!br.resource_reject && !br.direct) br.bank_skip = true;
      if (br.error.empty()) br.error = "INCOMPLETE_TWO_CURVE_BANK";
      br.rows_data.clear();
      br.rows_data.shrink_to_fit();
      br.curves.clear();
      br.curves.shrink_to_fit();
      return br;
    }
    if (br.rows_data.empty()) {
      br.bank_skip = true;
      br.error = "EMPTY_COMPLETE_BANK";
      br.curves.clear();
      br.curves.shrink_to_fit();
      return br;
    }
    ...
    br.gcd_free_splits = decoder.splits;
    ...
    br.block_records.reserve(decoder.blocks.size());
    for (const Block& block : decoder.blocks) {
      std::ostringstream exponents;
      bool first = true;
      for (int row = 0; row < static_cast<int>(block.exponents.size()); ++row) {
        if (block.exponents[row] == 0) continue;
        if (!first) exponents << ',';
        first = false;
        exponents << row << ':' << block.exponents[row];
      }
      br.block_records.push_back({block.value, squarez(block.value), exponents.str()});
    }
    if (!mine_chord_patterns(br, decoder.blocks)) {
      br.rows_data.clear();
      br.rows_data.shrink_to_fit();
      br.curves.clear();
      br.curves.shrink_to_fit();
    ...
    br.curves.clear();
    br.curves.shrink_to_fit();
  } catch (const ResourceReject& e) {
    br.eligible = false;
    br.resource_reject = true;
    br.relations = br.useful = br.strict_useful = 0;
    br.residual_relations = br.chord_relations = 0;
    br.residual_bank = false;
    br.saved.clear();
    br.rows_data.clear();
    br.rows_data.shrink_to_fit();
    br.curves.clear();
...
void write_certificate(std::ofstream& out, const Case& c, const Family& f,
                       const FactorLabel* factor_label,
                       bool direct, const Relation& rel) {
  out << "{\"version\":\"F265_V2\",\"split\":" << c.split
      << ",\"factor_bits\":" << c.factor_bits
      << ",\"shape\":" << json_string(shape_name(c.shape))
      << ",\"case_index\":" << c.index << ",\"family\":" << f.id
      << ",\"N\":" << json_string(dec(c.N));
  if (factor_label)
    out << ",\"p_label\":" << json_string(std::to_string(factor_label->p))
        << ",\"q_label\":" << json_string(std::to_string(factor_label->q));
  out
      << ",\"direct_before_relation\":" << (direct ? "true" : "false")
  ...
  for (size_t k = 0; k < rel.rows.size(); ++k) {
    if (k) out << ',';
    const CertificateRow& r = rel.rows[k];
    out << "{\"curve\":" << r.curve << ",\"index\":" << r.index
        << ",\"A\":" << json_string(dec(r.A))
        << ",\"B\":" << json_string(dec(r.B))
...
struct RunOutput {
  ...
  uint64_t rows = 0, pairs = 0, gcd_free_splits = 0, pattern_work = 0;
};
...
RunOutput run_cases(const std::vector<Case>& cases,
                    const std::vector<FactorLabel>* factor_labels,
                    const std::vector<int>& families, int workers,
                    const std::string& metrics_path, const std::string& cert_path,
                    const std::string& cells_path, const std::string& banks_path,
                    const std::string& blocks_path, const std::string& patterns_path) {
  if (factor_labels && factor_labels->size() != cases.size())
    throw std::runtime_error("factor-label cardinality mismatch");
  ...
  std::vector<PendingCertificate> pending_certificates;
  pending_certificates.reserve(64 * families.size());

  std::ofstream banks(banks_path, std::ios::binary);
  if (!banks) throw std::runtime_error("cannot open bank-status output");
  std::ofstream blocks(blocks_path, std::ios::binary);
  if (!blocks) throw std::runtime_error("cannot open opaque-block output");
  std::ofstream patterns(patterns_path, std::ios::binary);
  if (!patterns) throw std::runtime_error("cannot open pattern output");
  banks << "version\tsplit\tfactor_bits\tshape\tcase_index\tfamily\tN\teligible\tresource_reject"
           "\tbank_skip\tdirect\tfirst_factor\tfactor_kind\tstrict_relations\trows\tpairs\trelations"
           ...
           "\tgcd_free_splits\tpattern_work\tchord_block_pairs\tthird_root_tests\tthird_root_matches"
           "\tindex_sum\tindex_difference\tindex_twice\tindex_thrice\tindex_product"
           "\tindex_equal_v2\terror\n";
  blocks << "version\tsplit\tfactor_bits\tshape\tcase_index\tfamily\tN\tblock_id"
            "\tblock_value\texact_square\tfactorization_status\texponents\n";
  patterns << "version\tsplit\tfactor_bits\tshape\tcase_index\tfamily\tN\tblock_id"
              "\tblock_value\tchord_support\ttangent_support\tdiscriminant_support"
              "\tA\tB\tdelta\tcurve\tfirst_row\tsecond_row\tthird_row"
              "\tfirst_u\tsecond_u\tthird_u"
              "\ti\tj\tk\tthird_exists\tthird_root_match\ti_plus_j_eq_k"
              "\tabs_i_minus_j_eq_k\ttwo_i_eq_j\tthree_i_eq_j\ti_times_j_eq_k"
              "\tequal_binary_valuation\n";

  workers = std::max(1, std::min(workers, WORKERS_MAX));
  double evaluation_seconds = 0, evaluation_cpu_seconds = 0;
  uint64_t total_rows = 0, total_pairs = 0, total_splits = 0, total_pattern_work = 0;
  for (size_t batch_begin = 0; batch_begin < jobs.size();
       batch_begin += JOB_BATCH_MAX) {
    const size_t batch_size = std::min(JOB_BATCH_MAX, jobs.size() - batch_begin);
    std::vector<BankResult> results(batch_size);
    std::atomic<size_t> cursor{0};
    auto batch_start = std::chrono::steady_clock::now();
    std::clock_t batch_cpu_start = std::clock();
    std::vector<std::thread> pool;
    for (int thread_id = 0; thread_id < workers; ++thread_id) {
      pool.emplace_back([&cases, &jobs, &results, &cursor, batch_begin, batch_size] {
        for (;;) {
          size_t local = cursor.fetch_add(1);
          if (local >= batch_size) break;
          size_t job = batch_begin + local;
          results[local] =
              analyze_bank(cases[jobs[job].case_id], FAMILIES[jobs[job].family]);
        }
      });
    }
    for (auto& thread : pool) thread.join();
    evaluation_seconds += std::chrono::duration<double>(
        std::chrono::steady_clock::now() - batch_start).count();
    evaluation_cpu_seconds +=
        static_cast<double>(std::clock() - batch_cpu_start) / CLOCKS_PER_SEC;

    for (size_t local = 0; local < batch_size; ++local) {
      const size_t job = batch_begin + local;
      BankResult& br = results[local];
      const Case& current_case = cases[jobs[job].case_id];
      FamilyStats& family_stats = stats[br.family];
      ++family_stats.actual;
      family_stats.eligible += br.eligible;
      family_stats.resource_reject += br.resource_reject;
      family_stats.bank_skip += br.bank_skip;
      family_stats.direct_banks += br.direct;
      family_stats.strict_banks += br.strict_useful > 0;
      family_stats.strict_relations += br.strict_useful;
      family_stats.residual_banks += br.residual_bank;
      family_stats.residual_relations += br.residual_relations;
      family_stats.chord_relations += br.chord_relations;
      family_stats.rows += br.rows;
      family_stats.pairs += br.pairs;
      family_stats.all_relations += br.relations;
      family_stats.useful_relations += br.useful;
      family_stats.singleton_squares += br.singleton_squares;
      family_stats.equal_rows += br.equal_rows;
      family_stats.inverse_duplicates += br.inverse_duplicates;
      family_stats.square_multiple_pairs += br.square_multiple_pairs;
      family_stats.tangent_shared_pairs += br.tangent_shared_pairs;
      family_stats.chord_shared_pairs += br.chord_shared_pairs;
      family_stats.discriminant_shared_pairs += br.discriminant_shared_pairs;
      family_stats.gcd_free_splits += br.gcd_free_splits;
      family_stats.pattern_work += br.pattern_work;
      family_stats.chord_block_pairs += br.chord_block_pairs;
      family_stats.third_root_tests += br.third_root_tests;
      family_stats.third_root_matches += br.third_root_matches;
      family_stats.index_sum += br.index_sum;
      family_stats.index_difference += br.index_difference;
      family_stats.index_twice += br.index_twice;
      family_stats.index_thrice += br.index_thrice;
      family_stats.index_product += br.index_product;
      family_stats.index_equal_v2 += br.index_equal_v2;
      total_rows += br.rows;
      total_pairs += br.pairs;
      total_splits += br.gcd_free_splits;
      total_pattern_work += br.pattern_work;

      CellStats& cell =
          cells[{br.family, current_case.factor_bits, current_case.shape}];
      ++cell.actual;
      cell.eligible += br.eligible;
      cell.strict_banks += br.strict_useful > 0;
      banks << "F265_V2\t" << current_case.split << '\t'
            << current_case.factor_bits << '\t' << shape_name(current_case.shape)
            << '\t' << current_case.index << '\t' << br.family << '\t'
            << dec(current_case.N) << '\t' << br.eligible << '\t'
            << br.resource_reject << '\t' << br.bank_skip << '\t' << br.direct
            << '\t' << dec(br.first_factor) << '\t' << br.first_factor_kind
            << '\t' << br.strict_useful << '\t' << br.rows << '\t' << br.pairs
            << '\t' << br.relations << '\t' << br.singleton_squares << '\t'
            << br.equal_rows << '\t' << br.inverse_duplicates << '\t'
            << br.square_multiple_pairs << '\t' << br.tangent_shared_pairs
            << '\t' << br.chord_shared_pairs << '\t'
            << br.discriminant_shared_pairs << '\t' << br.gcd_free_splits
            << '\t' << br.pattern_work << '\t' << br.chord_block_pairs << '\t'
            << br.third_root_tests << '\t' << br.third_root_matches << '\t'
            << br.index_sum << '\t' << br.index_difference << '\t'
            << br.index_twice << '\t' << br.index_thrice << '\t'
            << br.index_product << '\t' << br.index_equal_v2 << '\t'
            << br.error << '\n';

      for (size_t block_id = 0; block_id < br.block_records.size(); ++block_id) {
        const OpaqueBlockRecord& block = br.block_records[block_id];
        blocks << "F265_V2\t" << current_case.split << '\t'
               << current_case.factor_bits << '\t'
               << shape_name(current_case.shape) << '\t' << current_case.index
               << '\t' << br.family << '\t' << dec(current_case.N) << '\t'
               << block_id << '\t' << dec(block.value) << '\t'
               << block.exact_square << "\tUNKNOWN_NOT_NEEDED\t"
               << block.exponents << '\n';
      }
      for (const PatternRecord& rec : br.patterns)
        patterns << "F265_V2\t" << current_case.split << '\t'
                 << current_case.factor_bits << '\t'
                 << shape_name(current_case.shape) << '\t' << current_case.index
                 << '\t' << br.family << '\t' << dec(current_case.N) << '\t'
                 << rec.block_id << '\t' << dec(rec.block_value) << '\t'
                 << dec(rec.support_value) << '\t' << dec(rec.tangent_support)
                 << '\t' << dec(rec.discriminant_support) << '\t' << dec(rec.A)
                 << '\t' << dec(rec.B) << '\t' << dec(rec.delta) << '\t'
                 << rec.curve << '\t' << rec.first_row << '\t'
                 << rec.second_row << '\t' << rec.third_row << '\t'
                 << dec(rec.first_u) << '\t' << dec(rec.second_u) << '\t'
                 << dec(rec.third_u) << '\t' << rec.i << '\t' << rec.j << '\t'
                 << rec.k << '\t' << rec.third_exists << '\t'
                 << rec.third_root_match << '\t' << rec.sum << '\t'
                 << rec.difference << '\t' << rec.twice << '\t' << rec.thrice
                 << '\t' << rec.product << '\t'
                 << rec.equal_binary_valuation << '\n';

      for (Relation& relation : br.saved) {
        if (saved_count[br.family] >= 64) break;
        pending_certificates.push_back(
            {jobs[job].case_id, br.family, br.direct, std::move(relation)});
        ++saved_count[br.family];
      }
    }
    if (!banks || !blocks || !patterns)
      throw std::runtime_error("public output stream failed during batch");
  }
  banks.close();
  blocks.close();
  patterns.close();
  if (!banks || !blocks || !patterns)
    throw std::runtime_error("public output stream failed");

  std::ofstream cert(cert_path, std::ios::binary);
  if (!cert) throw std::runtime_error("cannot open certificate output");
  for (const PendingCertificate& pending : pending_certificates) {
    const FactorLabel* factor_label =
        factor_labels ? &(*factor_labels)[pending.case_id] : nullptr;
    write_certificate(cert, cases[pending.case_id], FAMILIES[pending.family],
                      factor_label, pending.direct, pending.relation);
  }
  cert.close();
  if (!cert) throw std::runtime_error("certificate stream failed");

  std::ofstream metrics(metrics_path, std::ios::binary);
  if (!metrics) throw std::runtime_error("cannot open metrics output");
  metrics << "version\tfamily\tintended\tactual\teligible\tresource_reject\tbank_skip\tdirect_banks"
  ...
  for (int family : families) {
    const FamilyStats& family_stats = stats[family];
    metrics << "F265_V2\t" << family << '\t' << family_stats.intended << '\t'
            << family_stats.actual << '\t' << family_stats.eligible << '\t'
            << family_stats.resource_reject << '\t' << family_stats.bank_skip
            << '\t' << family_stats.direct_banks << '\t'
            << family_stats.strict_banks << '\t'
            << family_stats.strict_relations << '\t'
            << family_stats.residual_banks << '\t'
            << family_stats.residual_relations << '\t'
            << family_stats.chord_relations << '\t' << family_stats.rows << '\t'
            << family_stats.pairs << '\t' << family_stats.all_relations << '\t'
            << family_stats.useful_relations << '\t'
            << family_stats.singleton_squares << '\t' << family_stats.equal_rows
            << '\t' << family_stats.inverse_duplicates << '\t'
            << family_stats.square_multiple_pairs << '\t'
            << family_stats.tangent_shared_pairs << '\t'
            << family_stats.chord_shared_pairs << '\t'
            << family_stats.discriminant_shared_pairs << '\t'
            << family_stats.gcd_free_splits << '\t' << family_stats.pattern_work
            << '\t' << family_stats.chord_block_pairs << '\t'
            << family_stats.third_root_tests << '\t'
            << family_stats.third_root_matches << '\t' << family_stats.index_sum
            << '\t' << family_stats.index_difference << '\t'
            << family_stats.index_twice << '\t' << family_stats.index_thrice
            << '\t' << family_stats.index_product << '\t'
            << family_stats.index_equal_v2 << '\n';
  }
  metrics.close();
  if (!metrics) throw std::runtime_error("metrics stream failed");
  ...
  for (const auto& item : cells) {
    int family, bits, shape;
    std::tie(family, bits, shape) = item.first;
    const CellStats& cell = item.second;
    cell_out << "F265_V2\t" << family << '\t' << bits << '\t'
             << shape_name(shape) << '\t' << cell.intended << '\t'
             << cell.actual << '\t' << cell.eligible << '\t'
             << cell.strict_banks << '\n';
  }
  cell_out.close();
  if (!cell_out) throw std::runtime_error("cell stream failed");
  return {std::move(stats), evaluation_seconds, evaluation_cpu_seconds,
          peak_rss_kib(), total_rows, total_pairs, total_splits,
          total_pattern_work};
}
void write_cohort(const std::string& path, const std::vector<Case>& cases,
  ...
  out << "version\tshortfall\tsplit\tfactor_bits\tshape\tindex\tN\n";
  for (const Case& c : cases)
    out << "F265_V2\t" << shortfall << '\t' << c.split << '\t' << c.factor_bits
        << '\t' << shape_name(c.shape) << '\t' << c.index << '\t' << dec(c.N)
        << '\n';
}

void write_factor_labels(const std::string& path, const std::vector<Case>& cases,
                         const std::vector<FactorLabel>& labels) {
  if (cases.size() != labels.size())
    throw std::runtime_error("factor-label cardinality mismatch");
  std::ofstream out(path, std::ios::binary);
  if (!out) throw std::runtime_error("cannot open post-evaluation labels");
  out << "version\tsplit\tfactor_bits\tshape\tindex\tN\tp\tq\n";
  for (size_t i = 0; i < cases.size(); ++i)
    out << "F265_V2\t" << cases[i].split << '\t' << cases[i].factor_bits << '\t'
        << shape_name(cases[i].shape) << '\t' << cases[i].index << '\t'
        << dec(cases[i].N) << '\t' << labels[i].p << '\t' << labels[i].q << '\n';
}
...
std::vector<FamilyStats> read_metrics(const std::string& path) {
  ...
  while (std::getline(in, line)) {
    ...
    s >> version >> f >> x.intended >> x.actual >> x.eligible >> x.resource_reject
      >> x.bank_skip >> x.direct_banks >> x.strict_banks >> x.strict_relations >> x.residual_banks
      ...
      >> x.discriminant_shared_pairs >> x.gcd_free_splits >> x.pattern_work
      >> x.chord_block_pairs
      >> x.third_root_tests >> x.third_root_matches >> x.index_sum
      >> x.index_difference >> x.index_twice >> x.index_thrice
      >> x.index_product >> x.index_equal_v2;
    if (!s || version != "F265_V2" || f < 0 || f >= FAMILY_COUNT)
...
std::map<std::tuple<int, int, int>, CellStats> read_cells(const std::string& path) {
  ...
  while (std::getline(in, line)) {
    ...
    s >> version >> f >> bits >> shape >> x.intended >> x.actual >> x.eligible
      >> x.strict_banks;
    if (!s || version != "F265_V2") throw std::runtime_error("invalid cell row");
...
void summarize_heldout(const std::string& metrics_path,
  ...
  out << "{\"version\":\"F265_V2\",\"label\":" << json_string(label)
...
void self_test() {
  ...
  BankResult pattern_bank;
  pattern_bank.curves = {{0, 0, 6, 972, 0, 0}};
  pattern_bank.rows_data = {{0, 1, 1, 1, 7, 0},
                            {0, 2, 2, 1, 14, 0},
                            {0, 3, 4, 1, 70, 0}};
  std::vector<Block> pattern_blocks = {{7, {1, 1, 1}}};
  if (!mine_chord_patterns(pattern_bank, pattern_blocks) ||
      pattern_bank.third_root_tests != 3 ||
      pattern_bank.third_root_matches != 3 || pattern_bank.patterns.empty() ||
      !pattern_bank.patterns[0].sum || !pattern_bank.patterns[0].twice)
    throw std::runtime_error("chord-pattern self-test failed");
  if (modz(cpp_int(10402) + 1, cpp_int(10403)) != 0)
    throw std::runtime_error("POWER zero-A self-test failed");
  std::cout << "SELF_TEST_PASS p20_gcds=101,103 pooled=0 decoder_gcds=7,11 "
               "chord_third_roots=3 power_zero_A=1\n";
}

Corpus make_preflight_cases() {
  std::set<std::string> used;
  Corpus corpus;
  for (int bits : {12, 32, 60})
    for (int shape = 0; shape < 3; ++shape) {
      Case c;
      FactorLabel label;
      if (make_case(0, bits, shape, 0, used, c, label)) {
        corpus.cases.push_back(std::move(c));
        corpus.labels.push_back(label);
      } else {
        ++corpus.shortfall;
...
long double planned_bank_work(const Family& family, int nbits) {
  long double rows = planned_rows(family, nbits);
  long double pair_work = rows * rows;
  long double pattern_work = CHORD_TESTS_MAX;
  return std::max<long double>(1, pair_work + pattern_work);
}
...
long double planned_full_work() {
  ...
  for (int factor_bits : {12, 16, 20, 24, 32, 40, 48, 60}) {
    ...
    for (const auto& f : FAMILIES) {
      long double work = planned_bank_work(f, nbits);
      family_work.push_back(work);
      total += 3.0L * 16.0L * work;
    }
...
long double planned_preflight_work(const std::vector<Case>& cases) {
  ...
  for (const Case& c : cases)
    for (const auto& f : FAMILIES) {
      total += planned_bank_work(f, bitlen(c.N));
    }
  return total;
}
...
void preflight(const std::string& output, int workers) {
  ...
  std::clock_t total_cpu_start = std::clock();
  auto generation_start = std::chrono::steady_clock::now();
  std::clock_t generation_cpu_start = std::clock();
  Corpus corpus = make_preflight_cases();
  double generation_seconds = std::chrono::duration<double>(
      std::chrono::steady_clock::now() - generation_start).count();
  double generation_cpu_seconds =
      static_cast<double>(std::clock() - generation_cpu_start) / CLOCKS_PER_SEC;
  ...
  std::string blocks = output + ".blocks.tsv";
  std::string patterns = output + ".patterns.tsv";
  std::string labels = output + ".labels.tsv";
  auto analysis_start = std::chrono::steady_clock::now();
  RunOutput r = run_cases(corpus.cases, &corpus.labels, families, workers,
                          metrics, certs, cells, banks, blocks, patterns);
  write_factor_labels(labels, corpus.cases, corpus.labels);
  double analysis_output_seconds = std::chrono::duration<double>(
      std::chrono::steady_clock::now() - analysis_start).count();
  long double work_ratio = planned_full_work() / planned_preflight_work(corpus.cases);
  long double generation_ratio = 1536.0L /
      std::max<size_t>(1, corpus.cases.size());
  long double projected_evaluation =
      std::max<long double>(analysis_output_seconds, 0.001L) * work_ratio;
  long double projected_generation =
      std::max<long double>(generation_seconds, 0.001L) * generation_ratio;
  long double projected = (projected_evaluation + projected_generation) * 1.75L;
  double total_seconds = std::chrono::duration<double>(
      std::chrono::steady_clock::now() - total_start).count();
  double total_cpu_seconds =
      static_cast<double>(std::clock() - total_cpu_start) / CLOCKS_PER_SEC;
  uint64_t preflight_output_bytes = 0;
  for (const std::string& path : {metrics, certs, cells, banks, blocks, patterns, labels}) {
    std::ifstream in(path, std::ios::binary | std::ios::ate);
    if (!in) throw std::runtime_error("cannot size preflight output");
    std::streampos size = in.tellg();
    if (static_cast<std::streamoff>(size) < 0)
      throw std::runtime_error("invalid preflight output size");
    preflight_output_bytes +=
        static_cast<uint64_t>(static_cast<std::streamoff>(size));
  }
  long double projected_output_bytes =
      std::max<long double>(preflight_output_bytes, 1.0L) * work_ratio * 1.25L;
  bool pass = corpus.shortfall == 0 && projected <= 12600.0L &&
              r.peak_rss_kib <= 3670016ULL &&
              projected_output_bytes <= 805306368.0L;
  std::ofstream out(output, std::ios::binary);
  out << "{\"version\":\"F265_V2\",\"cases\":" << corpus.cases.size()
      << ",\"shortfall\":" << corpus.shortfall
      << ",\"families\":12,\"wall_seconds\":" << std::fixed << std::setprecision(6)
      << total_seconds << ",\"cpu_seconds\":" << total_cpu_seconds
      << ",\"generation_wall_seconds\":" << generation_seconds
      << ",\"generation_cpu_seconds\":" << generation_cpu_seconds
      << ",\"evaluation_wall_seconds\":" << r.seconds
      << ",\"evaluation_cpu_seconds\":" << r.cpu_seconds
      << ",\"analysis_and_output_wall_seconds\":" << analysis_output_seconds
      << ",\"rows\":" << r.rows << ",\"pair_controls\":" << r.pairs
      << ",\"gcd_free_splits\":" << r.gcd_free_splits
      << ",\"pattern_work\":" << r.pattern_work
      << ",\"work_ratio\":" << static_cast<double>(work_ratio)
      << ",\"generation_ratio\":" << static_cast<double>(generation_ratio)
      << ",\"safety_multiplier\":1.75,\"projected_full_seconds\":"
      << static_cast<double>(projected)
      << ",\"projected_evaluation_seconds\":"
      << static_cast<double>(projected_evaluation * 1.75L)
      << ",\"projected_generation_seconds\":"
      << static_cast<double>(projected_generation * 1.75L)
      << ",\"preflight_output_bytes\":" << preflight_output_bytes
      << ",\"projected_output_bytes\":"
      << static_cast<double>(projected_output_bytes)
      << ",\"projected_output_gate_bytes\":805306368"
...
int main(int argc, char** argv) {
  try {
    ...
    if (mode == "--discovery" || mode == "--heldout") {
      ...
      Corpus corpus = make_corpus(split);
      write_cohort(arg_value(argc, argv, "--cohort"), corpus.cases,
                   corpus.shortfall);
      ...
      RunOutput result = run_cases(corpus.cases, &corpus.labels, families, workers,
                                   arg_value(argc, argv, "--metrics"),
                                   arg_value(argc, argv, "--certificates"),
                                   arg_value(argc, argv, "--cells"),
                                   arg_value(argc, argv, "--banks"),
                                   arg_value(argc, argv, "--blocks"),
                                   arg_value(argc, argv, "--patterns"));
      write_factor_labels(arg_value(argc, argv, "--labels"), corpus.cases,
                          corpus.labels);
      std::cout << (split == 0 ? "DISCOVERY" : "HELDOUT")
                << " cases=" << corpus.cases.size()
                << " shortfall=" << corpus.shortfall
                << " wall_seconds=" << std::fixed << std::setprecision(6)
                << result.seconds << " cpu_seconds=" << result.cpu_seconds
                << " rows=" << result.rows << " pairs=" << result.pairs
                << " gcd_free_splits=" << result.gcd_free_splits
                << " pattern_work=" << result.pattern_work
                << " peak_rss_kib=" << result.peak_rss_kib << '\n';
      return 0;
    }
    throw std::runtime_error("unknown mode " + mode);
  } catch (const std::exception& e) {
    std::cerr << "F265_D02_ERROR " << e.what() << '\n';
    return 2;
  }
}
