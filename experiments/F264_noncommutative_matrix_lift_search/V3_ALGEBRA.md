# F264-D03 V3 exact algebra and certificate rules

V1 and its failed audit remain immutable. This file is the algebra contract
for the repaired V3 packet only. Every formula is unchanged from V2.
...
V3 records the exact decimal lcm. It defines its displayed new-bit count as
...
the gcd changes from `15` to `3`. V3 does not assert gcd invariance. It tests
...
are exact positive integers. V3 records both baseline-improvement factors,
not only the predicate that one exceeds one.

## Conjugate-basis control

For every word step V3 reports, as separate typed streams, all four entries
