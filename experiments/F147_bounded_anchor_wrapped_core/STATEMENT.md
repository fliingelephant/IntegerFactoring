# F147 candidate statement — bounded-anchor wrapped cycles have a quasipolynomial large-core decoder
...
Multiplying `L` anchors bounded by `H` can reach the P131 scale once

\[
L\ge \frac{\log N}{2\log H}.
\tag{12}
\]

This is polynomial length when `H=n^{O(1)}`.  But (12) is only a magnitude
condition.  It does not force a return edge, a square residual product, or a
non-global root.
