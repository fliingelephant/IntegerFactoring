#!/usr/bin/env python3
"""Named 900-second hard-timeout runner for F157-D01."""
...
TIMEOUT_SECONDS = 900
TIMEOUT_NAME = "F157_SPARSE_SECTION_FEEDBACK_CAPABILITY_HARD_TIMEOUT"
...
def main() -> int:
    ...
    if observed_hashes != registration["frozen_files"]:
        raise AssertionError("a preregistered F157 file changed")
    ...
    with log.open("w", encoding="utf-8") as stream:
        ...
        try:
            ...
            failure = directory / f"RUN_FAILED_{timestamp}_TIMEOUT.log"
        ...
        if completed.returncode:
            ...
            failure = directory / f"RUN_FAILED_{timestamp}_EXIT_{completed.returncode}.log"
            failure.write_bytes(log.read_bytes())
            return completed.returncode
        if not attempt.exists():
            stream.write("status=FAIL_MISSING_OUTPUT\n")
            failure = directory / f"RUN_FAILED_{timestamp}_MISSING_OUTPUT.log"
            failure.write_bytes(log.read_bytes())
            return 2
        output = json.loads(attempt.read_text())
        if output.get("status") != "PASS":
            stream.write("status=FAIL_OUTPUT_STATUS\n")
            failure = directory / f"RUN_FAILED_{timestamp}_OUTPUT_STATUS.log"
            failure.write_bytes(log.read_bytes())
            return 3
