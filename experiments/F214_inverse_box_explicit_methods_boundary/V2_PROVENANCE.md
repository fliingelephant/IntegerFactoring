...
## Preserved V1 and exact repair

V2 preserves every V1 file and HOSTILE_AUDIT.md byte-for-byte. The hostile
audit gave V1 a FAIL because a balanced-semiprime-only selector was used as
if its promise were recursively closed on \(K=(N-1)/2\). The audit also
identified a manifest-only overstatement: the unit scan has exactly
\(\varphi(K)\) states and a proved \(\Omega(\sqrt N)\) lower bound, not a
proved \(\Theta(\sqrt N)\) count.

V2 makes two exact repairs:

1. it states the \(K\) and \(E\) bit lengths only as local facts at the
   balanced node, and states the P183 recurrence only after independently
   granting a correct all-input recursive dispatch with the required child
   bounds at every node; and
2. its manifest distinguishes the two interval scans, each
   \(\Theta(\sqrt N)\), from the full unit scan, which has exactly
   \(\varphi(K)\) states and the explicit lower bound in (9).

No V1 audit conclusion is transferred to V2. V2 requires fresh review.
