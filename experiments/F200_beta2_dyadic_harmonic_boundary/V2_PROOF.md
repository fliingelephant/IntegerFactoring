# Proof of the F200 V2 candidate
...
The total extension degree is \(ef\geq f\), so it is at least
\(\operatorname{ord}_p(2)\).

Now fix the public integer-valued numerical-QP cap \(D(n)\), and invoke
P160--P161 from the beta-two normalized hard branch with
\(T\geq D(n)\).  The proof does not condition away a factor or a factored
exact common-order state: either is a separate algorithmic exit.  Consider
only P161's surviving outcome
...
P160 constructs \(y=4^{N^n}\), and P161 constructs
\(w=y^Q\), where \(Q\) is public.  Hence \(w=2^E\) for a public exponent
\(E\).  P161's surviving conclusion applies in every hidden component and,
in particular, at the same hidden rational prime \(p\):
...
The preceding local-field bound therefore excludes a primitive \(p\)-th
root of unity from every extension of \(\mathbb Q_2\) of degree at most
\(D(n)\), only on outcome (15).  This neither merges the two earlier
P160--P161 exits into the surviving branch nor excludes complex
approximation or an implicit encoding.
