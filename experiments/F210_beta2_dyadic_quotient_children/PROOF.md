...
it vanishes exactly when \(r=c\). This proves (14). At \(m=2\), the only
canonical odd residue is one. If \(N\equiv3\pmod4\), then
...
where the first equality again uses (61). Thus (24) holds. Moreover,
...
## 5. Equal finite 2-adic lift counts

Fix \(s\ge t+1\). There are exactly
...
The oriented integral distinction is exact. The only positive factor pair
of the promised semiprime satisfying
...
is \((X,Y)=(p,q)\). Its residues modulo \(h\) select the true child. The
false child cannot contain that oriented pair. In the coalesced case it can
contain \((q,p)\), but this reverses (70). Thus finite congruence feasibility
does not replace the integer orientation test.
...
\(N\). If \(d=1\), retain that copy of \(\ell\). Suppose no factor occurs.
At termination, for every prime \(\ell\mid E\), neither local order divides
\(E/\ell\), while both divide \(E\). Therefore the \(\ell\)-adic valuation
of each local order equals that of \(E\). This holds for every prime of
\(E\), so
...
A numerical-QP number of calls at the bound (77) is absorbed into the
...
For every fixed \(\rho>3/4\), equation (78) is at most \(\rho n\) after a
...
\(T(n)\le T(n-1)+Q(n)\); F210 supplies no rule that realizes it.
