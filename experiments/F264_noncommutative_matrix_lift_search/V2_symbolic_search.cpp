...
static SplitCheck check_split_certificate(const M2& full, const cpp_int& N,
  ...
  for (const cpp_int& s : split_root_proposals(low, lift, conjugate_low,
                                                conjugate_lift)) {
...
int main(int argc, char** argv) try {
  ...
  if (argc == 2 && std::string(argv[1]) == "--describe") {
    ...
    std::cout << "F264_D02_DESCRIPTION families=" << FAMILY_COUNT
              << " candidates=" << candidates.size() << " aliases=" << aliases
              << " words_per_input=8744 p205_words=467 planned_inputs=5280"
              << " discovery_inputs=2208 heldout_inputs=3072 tsv_columns=2541"
              << " max_threads=8\n";
    return 0;
  }
  ...
  require(argc == 6,
          "usage: V2_symbolic_search THREADS SPLIT OUT_JSON OUT_TSV OUT_ANOMALIES");
