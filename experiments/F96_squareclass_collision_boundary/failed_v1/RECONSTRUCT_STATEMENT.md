...
- the first useful square-class collision is \((a,b)=(99,1)\);
- it is one-based pair ordinal 5150 and one-based unique-residue ordinal
  299;
- it gives \(c=1263,w=1684,P=P_2\);
- the smaller menu \(0\le a,b\le n\) has no collision;
- all nonnegative raw integer monomials \(3^a43^b<N\), other than one,
  number 12 and have no collision with \(P_1\).

You may also independently verify that the complete subgroup \(H\) has
exactly two closing residues, \(1263\) and \(1684\), which form one inverse
pair and one relation value. Their density in \(H\) is \(1/667\).
