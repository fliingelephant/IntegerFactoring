...
work_dir=/root/F256_pell_carried_odd_pair_scan
...
expected_algebra_sha256=$2
expected_scan_sha256=$3
expected_runner_sha256=$4
test "$(sha256sum "$work_dir/PREREGISTRATION.md" | cut -d ' ' -f 1)" = "$expected_prereg_sha256"
test "$(sha256sum "$work_dir/ALGEBRA.md" | cut -d ' ' -f 1)" = "$expected_algebra_sha256"
...
ulimit -v 2097152
"$python_bin" "$work_dir/scan.py" \
  --self-test \
  --prereg-sha256 "$expected_prereg_sha256" \
  --algebra-sha256 "$expected_algebra_sha256" \
  >"$output_dir/SELF_TEST.stdout" 2>"$output_dir/SELF_TEST.stderr"
test "$(cat "$output_dir/SELF_TEST.stdout")" = SELF_TEST_PASS
test ! -s "$output_dir/SELF_TEST.stderr"
...
test "$(wc -l < "$output_dir/TRAIN.jsonl")" -eq 512
test "$(wc -l < "$output_dir/HELDOUT.jsonl")" -eq 512
test ! -s "$output_dir/RUN.stderr"
...
sha256sum \
  ...
  "$output_dir/SELF_TEST.stdout" \
  "$output_dir/SELF_TEST.stderr" \
