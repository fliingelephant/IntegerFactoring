...
constexpr u64 kMaxRelationPayload = 268435456ULL;
constexpr u64 kMaxOutputBytes = 536870912ULL;
...
Decoder decode_rows(const std::vector<Row>& rows) {
  ...
  bool changed=true;while(changed){changed=false;
    for(std::size_t i=0;i<d.blocks.size()&&!changed;++i)for(std::size_t j=i+1;j<d.blocks.size();++j){cpp_int g=gcdz(d.blocks[i].value,d.blocks[j].value);if(g==1)continue;
      ...
      ++d.steps;require(d.steps<=kMaxSteps,"GCD_FREE_STEPS_CAP");require(static_cast<int>(d.blocks.size())<=kMaxBlocks,"BLOCK_CAP");changed=true;break;}}
  }
...
CaseResult analyze_case(const Case& c,const std::array<std::vector<Row>,kFamilyCount>& family_rows) {
  ...
  for(const auto& input:original){auto [it,inserted]=retained.emplace(input.value,result.rows.size());
    ...
    for(const auto& prior:row.provenance)
      account_relation(result,make_duplicate_relation(c.index,row,prior,input.provenance[0],c.N),false);
  ...
  Decoder decoder=decode_rows(result.rows);result.refine_steps=decoder.steps;result.blocks=decoder.blocks;
  result.dedup_rank=decoder.rank;result.dedup_nullity=decoder.kernel.size();
  ...
  Decoder core_decoder=decode_rows(core);result.core_refine_steps=core_decoder.steps;result.core_rank=core_decoder.rank;result.core_nullity=core_decoder.kernel.size();
  ...
  for(int i=0;i<result.core_rows;++i){++result.singleton_tests;cpp_int root;if(!squarez(core[i].value,&root))continue;
    ...
    if(relation.root_class=="USEFUL")useful_low=true;else low.insert(bits);account_relation(result,std::move(relation),true);
  }
  for(int i=0;i<result.core_rows;++i)for(int j=i+1;j<result.core_rows;++j){++result.pair_tests;cpp_int root;if(!same_square_class(core[i].value,core[j].value,&root))continue;
  ...
  if(!useful_low){for(const auto& kernel:core_decoder.kernel){Bits remainder=low.reduce(kernel);if(bit_zero(remainder)||!low.insert(remainder))continue;
      ...
      account_relation(result,make_relation(c.index,"RESIDUAL",selected,c.N,remainder),true);}
...
std::map<std::string,std::string> serialize_results(const std::vector<CaseResult>& results,const std::string& mode) {
  std::map<std::string,std::string> files;
  std::ostringstream cases,rows,blocks,peeling,relations;
  cases<<"case_index\tcell_index\tN\tcohort\tshape\tfactor_bits\toriginal_rows\tdedup_rows\tpeeled_rows\tcore_rows"
  ...
  rows<<"case_index\tN\tcohort\tshape\tfactor_bits\tretained_row\tU\tY\tprovenance_count\tprovenance\n";
  blocks<<"case_index\tN\tcohort\tshape\tfactor_bits\tblock\tvalue\tsquare_class\texponents\tinitial_degree\tcore_degree\n";
  peeling<<"case_index\tN\tcohort\tshape\tfactor_bits\tround\tretained_row\twitness_blocks\tprovenance\n";
  relations<<"case_index\tN\tcohort\tshape\tfactor_bits\tsequence\tkind\tsupport\tfamily_class\tprovenance\texact_root\tsupplied_root"
             "\tnormalized_root\tgcd_minus\tgcd_plus\troot_class\n";
  u64 relation_payload=0;int relation_count=0;
  for(const auto& r:results){std::vector<std::string> family_ranks;for(int rank:r.family_ranks)family_ranks.push_back(std::to_string(rank));
    cases<<r.case_index<<'\t'<<r.cell_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'<<r.shape<<'\t'<<r.factor_bits<<'\t'
    ...
    for(const auto& row:r.rows)rows<<r.case_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'<<r.shape<<'\t'<<r.factor_bits<<'\t'<<row.retained_id<<'\t'
         <<row.value<<'\t'<<row.root<<'\t'<<row.provenance.size()<<'\t'<<row_provenance(row)<<'\n';
    for(int i=0;i<static_cast<int>(r.blocks.size());++i)blocks<<r.case_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'<<r.shape<<'\t'<<r.factor_bits<<'\t'<<i<<'\t'
    ...
    for(const auto& p:r.peeling)peeling<<r.case_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'<<r.shape<<'\t'<<r.factor_bits<<'\t'<<p.round<<'\t'
         <<p.row<<'\t'<<block_list(p.blocks)<<'\t'<<p.provenance<<'\n';
    for(const auto& relation:r.relations){std::ostringstream line;
      line<<r.case_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'<<r.shape<<'\t'<<r.factor_bits<<'\t'<<relation.sequence<<'\t'<<relation.kind<<'\t'
          <<relation.support<<'\t'<<relation.family_class<<'\t'<<relation.provenance<<'\t'<<relation.exact_root<<'\t'<<relation.supplied_root<<'\t'
          <<relation.normalized<<'\t'<<relation.gcd_minus<<'\t'<<relation.gcd_plus<<'\t'<<relation.root_class<<'\n';
      std::string text=line.str();relation_payload+=text.size();relations<<text;++relation_count;
    }
  }
  require(relation_count<=kMaxGlobalRelations,"GLOBAL_RELATION_CAP");require(relation_payload<=kMaxRelationPayload,"RELATION_PAYLOAD_CAP");
  files["cases.tsv"]=cases.str();files["rows.tsv"]=rows.str();files["blocks.tsv"]=blocks.str();files["peeling.tsv"]=peeling.str();files["relations.tsv"]=relations.str();

  struct Aggregate { int cases=0;u64 original=0,dedup=0,peeled=0,core=0,private_loss=0,pair_tests=0,relations=0,useful=0,pure=0,cross=0,pure_core=0,cross_core=0; };
  std::map<std::tuple<std::string,std::string,int,std::string>,Aggregate> aggregate;
  for(const auto& r:results){std::vector<std::tuple<std::string,std::string,int,std::string>> keys={
    ...
    for(const auto& key:keys){auto& a=aggregate[key];++a.cases;a.original+=r.original_rows;a.dedup+=r.dedup_rows;a.peeled+=r.peeled_rows;a.core+=r.core_rows;
      a.private_loss+=r.private_loss;a.pair_tests+=r.pair_tests;a.relations+=r.relations.size();a.useful+=r.useful_relations;a.pure+=r.pure_relations;a.cross+=r.cross_relations;
      a.pure_core+=r.pure_core_relations;a.cross_core+=r.cross_core_relations;}}
  std::ostringstream aggregates;aggregates<<"scope\tcohort\tfactor_bits\tshape\tcases\toriginal_rows\tdedup_rows\tpeeled_rows\tcore_rows\tprivate_pivot_loss"
      "\tsupport_two_tests\trelations\tuseful_relations\tpure_relations\tcross_relations\tpure_core_relations\tcross_core_relations\n";
  for(const auto& [key,a]:aggregate){const auto& [scope,cohort,bits,shape]=key;aggregates<<scope<<'\t'<<cohort<<'\t'<<(bits<0?"ALL":std::to_string(bits))<<'\t'<<shape<<'\t'
      <<a.cases<<'\t'<<a.original<<'\t'<<a.dedup<<'\t'<<a.peeled<<'\t'<<a.core<<'\t'<<a.private_loss<<'\t'<<a.pair_tests<<'\t'<<a.relations<<'\t'
      <<a.useful<<'\t'<<a.pure<<'\t'<<a.cross<<'\t'<<a.pure_core<<'\t'<<a.cross_core<<'\n';}
  files["aggregates.tsv"]=aggregates.str();

  std::vector<const RelationRecord*> candidates;std::map<int,const CaseResult*> by_case;
  for(const auto& r:results){by_case[r.case_index]=&r;for(const auto& relation:r.relations)if(relation.root_class=="USEFUL")candidates.push_back(&relation);}
  std::string category="USEFUL_RELATION";
  if(candidates.empty()){category="CROSS_FAMILY_RELATION";for(const auto& r:results)for(const auto& relation:r.relations)if(relation.family_class=="CROSS_FAMILY")candidates.push_back(&relation);}
  auto relation_less=[&](const RelationRecord* a,const RelationRecord* b){const auto& ca=*by_case.at(a->case_index);const auto& cb=*by_case.at(b->case_index);
    return std::tie(ca.N,a->case_index,a->kind,a->support,a->provenance)<std::tie(cb.N,b->case_index,b->kind,b->support,b->provenance);};
  std::sort(candidates.begin(),candidates.end(),relation_less);
  std::ostringstream witness;witness<<"category\tcase_index\tN\tcohort\tshape\tfactor_bits\tkind\tsupport\tfamily_class\tprovenance\texact_root\tsupplied_root"
      "\tnormalized_root\tgcd_minus\tgcd_plus\troot_class\n";
  if(!candidates.empty()){const auto& relation=*candidates.front();const auto& r=*by_case.at(relation.case_index);witness<<category<<'\t'<<r.case_index<<'\t'<<r.N<<'\t'<<r.cohort<<'\t'
      <<r.shape<<'\t'<<r.factor_bits<<'\t'<<relation.kind<<'\t'<<relation.support<<'\t'<<relation.family_class<<'\t'<<relation.provenance<<'\t'
      <<relation.exact_root<<'\t'<<relation.supplied_root<<'\t'<<relation.normalized<<'\t'<<relation.gcd_minus<<'\t'<<relation.gcd_plus<<'\t'<<relation.root_class<<'\n';
  }else{const CaseResult* chosen=nullptr;category="NONEMPTY_CORE";for(const auto& r:results)if(r.core_rows&&(!chosen||std::tie(r.N,r.case_index)<std::tie(chosen->N,chosen->case_index)))chosen=&r;
    if(!chosen){category="PIVOT_LOSS";for(const auto& r:results)if(r.private_loss&&(!chosen||std::tie(r.N,r.case_index)<std::tie(chosen->N,chosen->case_index)))chosen=&r;}
    if(chosen)witness<<category<<'\t'<<chosen->case_index<<'\t'<<chosen->N<<'\t'<<chosen->cohort<<'\t'<<chosen->shape<<'\t'<<chosen->factor_bits<<"\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\n";
    else witness<<"NONE\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\n";}
  files["witness.tsv"]=witness.str();

  int clean_cases=0,control_cases=0,clean_nonempty=0,clean_cross_core=0,total_core=0,total_useful=0,total_relations=0;
  std::map<std::pair<int,std::string>,int> clean_loss_cells;
  for(const auto& r:results){bool clean=r.cohort=="CLEAN";clean_cases+=clean;control_cases+=!clean;total_core+=r.core_rows;total_useful+=r.useful_relations;total_relations+=r.relations.size();
    clean_nonempty+=clean&&r.core_rows>0;clean_cross_core+=clean&&r.core_rows>0&&r.cross_core_relations>0;if(clean&&r.private_loss>0)++clean_loss_cells[{r.factor_bits,r.shape}];}
  bool repeated_loss=false;for(const auto& [cell,count]:clean_loss_cells)repeated_loss|=count>=2;
  std::string status;
  if(mode=="preflight")status="PREFLIGHT_ONLY";
  else if(total_core==0)status="POSTHOC_EMPTY_CORE_KILL";
  else if(clean_cases==kCleanCases&&clean_cross_core>0)status="POSTHOC_CROSS_CORE_LEAD";
  else if(clean_cases==kCleanCases&&clean_nonempty>0&&repeated_loss)status="POSTHOC_REPEATABLE_PIVOT_LOSS";
  else status="POSTHOC_NULL";
  std::ostringstream summary;summary<<"key\tvalue\nanalysis_mode\t"<<mode<<"\nevidence_class\tPOSTHOC_DISCOVERY_ONLY\ncases\t"<<results.size()<<"\nclean_cases\t"<<clean_cases<<"\ncontrol_cases\t"<<control_cases
      <<"\noriginal_rows\t"<<u64(results.size())*kOriginalRows<<"\ncore_rows\t"<<total_core<<"\nclean_nonempty_cores\t"<<clean_nonempty
      <<"\nclean_cross_core_cases\t"<<clean_cross_core<<"\nrepeatable_clean_pivot_loss_cell\t"<<repeated_loss<<"\nrelations\t"<<total_relations
      <<"\nuseful_relations\t"<<total_useful<<"\nstatus\t"<<status<<"\ninterpretation\tFINITE_POSTHOC_HYPOTHESIS_GENERATION_NOT_A_THEOREM\n";
  files["summary.tsv"]=summary.str();
  u64 total=0;for(const auto& [name,text]:files)total+=text.size();require(total<=kMaxOutputBytes,"OUTPUT_BYTES_CAP");return files;
}

void write_outputs(const std::string& directory,const std::map<std::string,std::string>& files) {
  require(!std::filesystem::exists(directory),"output directory exists");
  std::filesystem::create_directories(directory);
  for(const auto& [name,text]:files){std::ofstream out(std::filesystem::path(directory)/name,std::ios::binary);require(bool(out),"open output "+name);out<<text;require(bool(out),"write output "+name);}
}
...
int main(int argc,char** argv) {
  try {
    ...
    for(const char* key:{"--mode","--evidence","--banks","--corpus","--out-dir","--workers"})require(args.count(key),std::string("missing ")+key);
    require(args.size()==6,"unknown CLI key");std::string mode=args["--mode"];require(mode=="preflight"||mode=="target","mode");
    ...
    std::vector<CaseResult> results(selected.size());std::atomic<std::size_t> next{0};std::exception_ptr error;std::mutex error_mutex;
    auto worker=[&](){try{for(;;){std::size_t slot=next.fetch_add(1);if(slot>=selected.size())break;const Case& c=cases[selected[slot]];
          results[slot]=analyze_case(c,banks.at({c.shape,c.factor_bits,c.index}));}}
      catch(...){std::lock_guard<std::mutex> lock(error_mutex);if(!error)error=std::current_exception();next.store(selected.size());}};
    std::vector<std::thread> pool;for(int i=0;i<std::min<int>(workers,selected.size());++i)pool.emplace_back(worker);for(auto& thread:pool)thread.join();if(error)std::rethrow_exception(error);
    std::sort(results.begin(),results.end(),[](const CaseResult& a,const CaseResult& b){return a.case_index<b.case_index;});
    auto files=serialize_results(results,mode);std::ostringstream metadata;metadata<<"key\tvalue\nversion\tF270-D01\nmode\t"<<mode<<"\nworkers\t"<<workers
      <<"\nevidence_sha256\t"<<kEvidenceSha<<"\nbanks_sha256\t"<<kBanksSha<<"\ncorpus_sha256\t"<<kCorpusSha<<"\nselected_case_indices\t";
    for(std::size_t i=0;i<selected.size();++i){if(i)metadata<<',';metadata<<selected[i];}metadata<<"\n";files["metadata.tsv"]=metadata.str();
    u64 output_bytes=0;for(const auto& [name,text]:files)output_bytes+=text.size();require(output_bytes<=kMaxOutputBytes,"OUTPUT_BYTES_CAP");write_outputs(args["--out-dir"],files);
    std::cout<<"F270_OK mode="<<mode<<" cases="<<results.size()<<" out="<<args["--out-dir"]<<'\n';return 0;
