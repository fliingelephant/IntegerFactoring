...
def raycast(N,p,v):
    ...
    # An exact integer entry root also changes membership at r-1 to r.
    for j in sorted({z for r in roots for z in (r-1,r,r+1) if z>=0}):
...
if __name__ == '__main__':
    ...
    for N,k in ((10541,4),(147053,6)):
        ...
        for u in range(1,1<<h,2):
            ...
            print('validated',N,k,u,counts,file=sys.stderr,flush=True)
    ...
    for N,k,a,b in ((147053,8,465,317),(147053,12,1,1),(147053,12,3,2)):
        ...
        while queue:
            ...
            if p[0]*p[1]%M==N%M:
                ...
                if best is None or (lower,p[0]*p[1],p)<(a*best[0]+b*best[1],best[0]*best[1],best):
                    best=p
                # The chosen point also minimizes product on the entire coarse
                # optimal face, so acceptance preserves this tie criterion.
                continue
            assert r<h
        ...
        print('hierarchy',N,k,a,b,visited,file=sys.stderr,flush=True)
