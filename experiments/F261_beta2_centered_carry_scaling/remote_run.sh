...
experiment=F261-D01
workers=4
if [[ ${F261_WORKERS:-} =~ ^[1-8]$ ]]; then
  workers=$F261_WORKERS
fi
...
g++ -std=c++17 -O3 -pthread -Wall -Wextra -Wconversion search.cpp -o search
./search --self-test
(
  ulimit -v 4194304
  exec timeout 3600s nice -n 15 ./search --preflight
) | tee "logs/${experiment}.preflight.txt"

(
  ulimit -v 4194304
  exec timeout 14400s nice -n 15 ./search "${workers}" \
