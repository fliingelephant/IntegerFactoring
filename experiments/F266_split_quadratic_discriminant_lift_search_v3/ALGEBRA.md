# F266-D03 algebra — canonical split-quadratic discriminant lifts
...
F266-D03 is a narrow repair of the immutable failed F266-D02 packet. D01 and
D02 and their hostile FAIL audits remain unchanged. D03 keeps the same eight
base families, five generators, word caps, two lift levels, direct tickets,
decoys, and P66 decoder. It changes no searched algebraic family.
...
For each public base ID, D03 chooses one gauge triple
...
2. `gcd` of every base coefficient and base discriminant with `N`;
3. `gcd` of every transformed coefficient and discriminant with `N`;
4. `gcd` of every projective root coordinate and every projective root
   determinant with `N`;
5. the exact Sylvester resultant of every retained pair of affine quadratic
   coefficient triples, followed by `gcd(resultant,N)`;
6. every single invariant carry (6), same-level quotient (7), and admitted
   cross-level quotient;
7. every exact singleton-square normalized-root comparison; and
8. every equal-row or exact square-multiple normalized-root comparison.
...
D03 executes these as eight global stages. It does not interleave a later
...
D03 retains four counters for each stage: total gcd tests, unit gcds, full
...
D03 serializes one residual row map per eligible bank and every final block
