...
- Date: 2026-08-08.
- Status: corrected exact bounded result. The first candidate failed hostile
  audit and proof-blind reconstruction on collision counts. The next
  candidate passed reconstruction but failed a fresh hostile audit on one
  normalization term. The current final wording has not been re-audited.
...
- Second failed result and manifest: `RESULT_FAILED_V2.md` and
  `RUN_MANIFEST_FAILED_V2.md`.
- Second hostile audit: `FINAL_AUDIT.md`.
- Corrected proof-blind reconstruction: `RECONSTRUCT_RESULT.md`.
