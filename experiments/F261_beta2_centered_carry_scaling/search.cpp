...
struct InputResult {
  ...
  u64 odd_min_abs;
  u64 odd_min_u;
  std::array<u64, 4> hit_count;
  std::array<u64, 4> first_hit;
  u64 half_ties;
...
static InputResult process_input(const Task& task) {
  ...
  for (u64 u = 1; u <= U3; ++u) {
    ...
    for (int j = 0; j < 4; ++j) {
      if (ac <= C[j]) {
        ++result.hit_count[j];
        if (result.first_hit[j] == 0) result.first_hit[j] = u;
      }
    }
  ...
  for (int pi = 0; pi < PREFIX_COUNT; ++pi) {
    ...
    const u64 a = p & (R - 1);
    const u64 b = R == 1 ? 0 : mul_mod(static_cast<u64>(N % R), inverse_power_two(a, R), R);
    ...
    for (u64 u = 1; u <= U2; ++u) {
      ...
      const u64 k0 = rounded_center(static_cast<u128>(u) * sqrtN, B);
  ...
  for (u64 u = 1; u <= 64; ++u) {
    const auto& point = first64[u - 1];
    const u64 common = rounded_center(static_cast<u128>(u) * sqrtN, B);
    const auto pi = center_intervals(N, B, u, true);
    const auto qi = center_intervals(N, B, u, false);
...
static void write_inverse_studies(const std::vector<InputResult>& results,
  ...
  out << "factor_bits\tcohort\tp\tq\tprefix\tt\tu\tmap_size\tmode"
         ...
         "\tmax_v2\thit_n\thit_n2\thit_n3\thit_cqp\n";
  for (const auto& [key, rp] : worst) {
    ...
    for (int pi = 1; pi < PREFIX_COUNT; ++pi) {
      ...
      for (u64 j = 0; j < public_count; ++j) {
        ...
        for (int k = 0; k < 4; ++k) if (ac <= thresholds[k]) ++hits[k];
      }
...
static bool better_score(const SelectionScore& a, const SelectionScore& b) {
  ...
  if (a.median2 != b.median2) return a.median2 < b.median2;
...
static std::vector<std::array<int, 3>> select_predictors(
  ...
  for (int pi = 0; pi < PREFIX_COUNT; ++pi) {
    ...
    for (int pred = 0; pred < PREDICTOR_COUNT; ++pred) {
      ...
      const u64 median2 = values.size() & 1 ? 2 * values[mid]
                                            : values[mid - 1] + values[mid];
      scores.push_back({min_hits, min_count, total, median2,
...
static void write_rows(const std::vector<InputResult>& results,
  ...
  out << "factor_bits\tsplit\tcohort\tindex\tattempts\tp\tq\tN\tn\tB\tH"
         ...
         "\thit_n\thit_n2\thit_n3\thit_cqp\tfirst_n\tfirst_n2\tfirst_n3"
         "\tfirst_cqp\thalf_ties\tlongest_diff_run\tmax_c_fibre\tzero_count"
  ...
  for (const auto& r : results) {
    ...
    for (u64 v : r.hit_count) out << '\t' << v;
    for (u64 v : r.first_hit) out << '\t' << v;
...
static void decoder_self_test() {
  ...
  require(monomial_exponents(2).size() == 15 && monomial_exponents(3).size() == 35,
          "monomial counts");
  std::cout << "F261_SELF_TEST_PASS N=" << string_u128(N)
            << " tie=1 negative=1\n";
}
...
static void preflight() {
  ...
  while (true) {
    ...
    if (q >= (1ULL << 59) && q > p && q < 2 * p) {
      ...
      const double inner_projection = 3.0 * seconds * frozen_evaluations
                                      / inner_iterations / 4.0;
      std::cout << "PREFLIGHT_INNER iterations=" << inner_iterations
                << " seconds=" << seconds << " sink=" << sink
                << " projected_four_worker_seconds=" << inner_projection << "\n";
      ...
      require(total <= 12000.0, "preflight projection exceeds 12000 seconds");
      return;
    }
    p += 2;
  }
...
int main(int argc, char** argv) try {
  ...
  require(argc == 6,
          "usage: search THREADS SUMMARY_JSON ROWS_TSV INVERSE_TSV CONSECUTIVE_TSV");
  ...
  write_summary(results, attempts, selected, generation_seconds, scan_seconds,
                threads, argv[2]);
  std::cout << "F261_D01_PASS inputs=" << results.size()
            << " generation_seconds=" << generation_seconds
            << " scan_seconds=" << scan_seconds << "\n";
  return 0;
} catch (const std::exception& e) {
  std::cerr << "F261_D01_FAIL " << e.what() << "\n";
  return 1;
}
