...
std::string corpus_bytes(const std::vector<Case>& cases) {
  ...
  for (const Case& c:cases)
    out<<"F266-D03\t"<<c.split<<'\t'<<c.shape<<'\t'<<c.factor_bits<<'\t'
       <<c.index<<'\t'<<c.N<<'\n';
  return out.str();
}
...
void emit_certificate_json(std::ostringstream& out,const Case& c,int family,
  ...
  out<<"{\"version\":\"F266-D03\",\"split\":"<<json_string(c.split)
...
std::string opaque_bytes(const std::vector<Case>& cases,
  ...
  for (const Evaluated& evaluated:rows) {
    const Case& c=cases[evaluated.case_index];const BankResult& b=evaluated.bank;
    if (b.eligible) {
      out<<"{\"version\":\"F266-D03\",\"kind\":\"ROW_MAP\",\"split\":"
    ...
    for (std::size_t block_index=0;block_index<b.opaque_blocks.size();++block_index) {
      ...
      out<<"{\"version\":\"F266-D03\",\"kind\":\"OPAQUE_BLOCK\",\"split\":"
         <<json_string(c.split)
         <<",\"shape\":"<<json_string(c.shape)<<",\"factor_bits\":"<<c.factor_bits
         <<",\"index\":"<<c.index<<",\"N\":"<<json_string(c.N.convert_to<std::string>())
...
Selection parse_selection(const std::string& bytes,const std::string& expected_digest) {
  ...
  require(bool(std::getline(input,line))&&line=="F266-D03-selection-v1","selection header");
...
std::set<std::string> parse_discovery_corpus(const std::string& bytes) {
  ...
  while (std::getline(input,line)) {
    ...
    std::istringstream row(line);std::vector<std::string> fields;std::string field;
    while (std::getline(row,field,'\t')) fields.push_back(field);
    require(fields.size()==6&&fields[0]=="F266-D03"&&fields[1]=="discovery",
...
void run_discovery(const std::string& directory,int workers) {
  ...
  const std::string corpus_name="F266-D03.discovery.corpus.tsv";
  const std::string corpus_sha_name="F266-D03.discovery.corpus.sha256";
  const std::string bank_name="F266-D03.discovery.banks.tsv";
  const std::string family_name="F266-D03.discovery.families.tsv";
  const std::string certificate_name="F266-D03.discovery.certificates.jsonl";
  const std::string opaque_name="F266-D03.discovery.opaque.jsonl";
  const std::string selection_name="F266-D03.selection.tsv";
  const std::string selection_sha_name="F266-D03.selection.sha256";
  ...
  write_text(join_path(directory,"F266-D03.discovery.manifest.tsv"),manifest_bytes(directory,names));
...
void run_heldout(const std::string& directory,const std::string& selection_path,
  ...
  auto start=std::chrono::steady_clock::now();
  for (const char* name:{"F266-D03.discovery.corpus.tsv","F266-D03.discovery.corpus.sha256",
                         "F266-D03.discovery.banks.tsv","F266-D03.discovery.families.tsv",
                         "F266-D03.discovery.certificates.jsonl","F266-D03.discovery.opaque.jsonl",
                         "F266-D03.selection.tsv",
                         "F266-D03.selection.sha256","F266-D03.discovery.manifest.tsv"})
  ...
  const std::string corpus_name="F266-D03.heldout.corpus.tsv";
  const std::string bank_name="F266-D03.heldout.banks.tsv";
  const std::string family_name="F266-D03.heldout.families.tsv";
  const std::string certificate_name="F266-D03.heldout.certificates.jsonl";
  const std::string opaque_name="F266-D03.heldout.opaque.jsonl";
  const std::string lead_name="F266-D03.heldout.lead_gate.tsv";
  write_text(join_path(directory,corpus_name),corpus);
  write_text(join_path(directory,bank_name),banks);
  write_text(join_path(directory,family_name),families);
  write_text(join_path(directory,certificate_name),certificates);
  ...
  write_text(join_path(directory,"F266-D03.heldout.manifest.tsv"),manifest_bytes(directory,names));
...
void run_preflight(const std::string& output_path,int workers) {
  ...
  json<<"{\"version\":\"F266-D03\",\"pass\":"<<(pass?"true":"false")
