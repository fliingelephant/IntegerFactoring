...
gcd calls per success the corresponding count is 28. Generation is excluded
...
The active comparison charges both positive child gcds before returning,
matching frozen F334/F336. The original early-return version and all its
outputs are retained under prior_early_return. That version had the same
success probabilities and query counts, but a different gcd-cost convention.
Its implementation mismatch was found during the paired scale implementation;
the corrected pilot and full small-input calculation both passed.

The scale took 0.524 seconds and peaked at 27,607,040 bytes RSS. Evidence:
