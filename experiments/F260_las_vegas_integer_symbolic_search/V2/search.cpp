...
static Grammar build_grammar() {
  ...
  std::sort(raw_seq.begin(), raw_seq.end(), [](const auto& a, const auto& b) {
    return syntax_order(a.depth,a.name,b.depth,b.name);
  });
  Grammar g;
  std::set<std::array<u64,3>> seen_seq;
  for (auto& p : raw_seq) if (seen_seq.insert(p.fingerprint).second) {
    g.seq.push_back(std::move(p));
    if (g.seq.size() == MAX_PROGRAMS_PER_TYPE) break;
  }
  ...
  for (int i = 0; i < static_cast<int>(g.seq.size()); ++i)
    for (int j = i + 1; j < static_cast<int>(g.seq.size()); ++j) {
      ...
      const std::string name = "word:" + g.seq[i].name + "*" + g.seq[j].name;
      const bool sparse = syntax_hash(name) % 5 == 0;
      if (same_base || both_id || sparse) raw_word.push_back({i,j,2,name,{}});
    }
  std::sort(raw_word.begin(), raw_word.end(), [](const auto& a, const auto& b) {
    return syntax_order(a.depth,a.name,b.depth,b.name);
  });
  std::set<std::array<u64,3>> seen_word;
  for (auto& p : raw_word) {
    p.fingerprint = fingerprint_word(p,g.seq,synthetic,residues);
    if (seen_word.insert(p.fingerprint).second) {
      g.word.push_back(std::move(p));
      if (g.word.size() == MAX_PROGRAMS_PER_TYPE) break;
    }
  }
  ...
  std::sort(raw_fact.begin(), raw_fact.end(), [](const auto& a, const auto& b) {
    return syntax_order(a.depth,a.name,b.depth,b.name);
  });
  std::set<std::array<u64,3>> seen_fact;
  for (auto& p : raw_fact) if (seen_fact.insert(p.fingerprint).second)
...
static void add_metrics(Cell& c, const Metrics& m, bool keep_collision,
                        bool keep_score, bool is_sequence, bool is_factored) {
...
static u64 write_aggregates(const std::string& path, const std::string& split,
  ...
  f.line("id\tsyntax\tkind\toperational\toracle\tsplit\tfactor_bits\tcohort\tcount"
         ...
         "\tmean_log_rp\tmean_log_rq\tmean_baseline_improvement"
  ...
  for (std::size_t id = 0; id < g.all.size(); ++id) for (int bi = 0; bi < nbits; ++bi)
    for (int cohort = 0; cohort < 3; ++cohort) {
      ...
      line << std::setprecision(18)
           ...
           << c.residual_log_q/den << '\t' << c.baseline_improvement/den << '\t'
...
static std::vector<std::string> discovery_files(bool manifest) {
  ...
  if (manifest) names.push_back("F260-D02.discovery.manifest.tsv");
  return names;
}
...
static std::vector<std::string> heldout_files(bool manifest) {
  ...
  if (manifest) names.push_back("F260-D02.heldout.manifest.tsv");
  return names;
}
...
static void run_discovery(const std::string& directory, int workers,
  ...
  write_grammar(join_path(directory,"F260-D02.grammar.tsv"),grammar,budget);
  const std::vector<CorpusRow> corpus = make_corpus(true,tiny);
  require(corpus.size() == (tiny ? 18U : 8960U), "discovery corpus count");
  const std::string corpus_path =
      join_path(directory,"F260-D02.discovery.corpus.tsv");
  write_corpus(corpus_path,corpus,budget);
  const std::string corpus_digest = sha256_file(corpus_path);
  write_digest(join_path(directory,"F260-D02.discovery.corpus.sha256"),
               corpus_digest,"F260-D02.discovery.corpus.tsv",budget);

  const CorpusEvaluation result =
      evaluate_corpus(corpus,grammar,workers,true,false);
  write_aggregates(join_path(directory,"F260-D02.discovery.aggregates.tsv"),
                   "discovery",grammar,result.aggregate,DISCOVERY_BITS,3,budget);
  write_certificates(join_path(directory,"F260-D02.discovery.certificates.tsv"),
                     "discovery",result.certificates,budget);
  write_collisions(join_path(directory,"F260-D02.discovery.collisions.tsv"),
                   "discovery",grammar,result.aggregate,DISCOVERY_BITS,3,budget);
  write_oracles(join_path(directory,"F260-D02.discovery.oracle.tsv"),
                grammar,result.aggregate,budget);
  const std::vector<int> selected =
      select_programs(grammar,result.aggregate);
  const std::string selection_path =
      join_path(directory,"F260-D02.selection.tsv");
  ...
  write_digest(join_path(directory,"F260-D02.selection.sha256"),
               selection_digest,"F260-D02.selection.tsv",budget);

  const auto records = discovery_files(false);
  require_output_names(directory,records);
  write_manifest(join_path(directory,"F260-D02.discovery.manifest.tsv"),
...
static void run_heldout(const std::string& directory,
  ...
  require(selection_path == join_path(directory,"F260-D02.selection.tsv"),
  ...
  write_corpus(join_path(directory,"F260-D02.heldout.corpus.tsv"),corpus,budget);
  const CorpusEvaluation result =
      evaluate_corpus(corpus,grammar,workers,true,true);
  write_aggregates(join_path(directory,"F260-D02.heldout.aggregates.tsv"),
                   "heldout",grammar,result.aggregate,HELDOUT_BITS,4,budget);
  write_certificates(join_path(directory,"F260-D02.heldout.certificates.tsv"),
                     "heldout",result.certificates,budget);
  write_collisions(join_path(directory,"F260-D02.heldout.collisions.tsv"),
                   "heldout",grammar,result.aggregate,HELDOUT_BITS,4,budget);
  write_lead_gate(join_path(directory,"F260-D02.heldout.lead_gate.tsv"),
                  grammar,result.aggregate,budget);
  ...
  write_manifest(join_path(directory,"F260-D02.heldout.manifest.tsv"),
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    print_process_resources("F260_D02",start);
    return 0;
  } catch (const std::exception& e) {
    std::cerr << "F260_D02_ERROR " << e.what() << '\n';
    return 1;
  }
}
