# Proof of the F153 V3 Jacobi-torus character boundary
...
`v`, their pointwise quotient projects to `v-v=0`. Thus
...
Harvey--Hittmeir large-order certificate in both components. On the split
side, choosing `d^2=D` gives the standard algebraic coordinate splitting of
`A_D`. This does not assert that an abstract isomorphism of finite cyclic
point groups needs the coordinate square root `d`.
...
lift. Choosing a parameter `t=f(alpha)` does not by itself classify the
resulting map as homomorphic or non-homomorphic. Without a separate
homomorphism or order proof, the ordinary order certificate gives no
torus-order theorem for that map. No conclusion follows for a new
torus-native high-order algorithm or for a coordinate map with a separately
proved torus-order law.

## 6. Scope

The proof classifies the stated direct operations only. The full
biquadratic algebra is a natural universal algebra containing both named
quadratic coordinates before relative norm. No absolute or unique minimality
claim is made. The proof does not give a lower bound for all coordinate
algorithms, and it does not exclude a
