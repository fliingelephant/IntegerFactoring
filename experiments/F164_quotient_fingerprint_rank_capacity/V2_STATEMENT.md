...
keeps a complete quasipolynomially obtainable factorization of `D`. Set

\[
B'=\max(B,\kappa(S)).
\]

Every prime factor of `M` is at most `B`, and every prime factor of
`kappa(S)` is at most `kappa(S)`. Thus every prime factor of `D=M kappa(S)`
is at most `B'`. The cap `B'` is quasipolynomial when the explicit bank is.
Thus `(h,D)` with scan cap `B'` is a valid next F161 state.
...
have the following self-contained legal provenance. For this certificate, a
canonical-inverse endpoint relation is the exact integer identity obtained
from a unit endpoint and its least positive inverse modulo `N`. Start with
the two endpoint pairs `(337,85)` and `(325,277)` in (14). Then perform
complete joint gcd-free refinement on all four positive endpoints: split by
every nontrivial pairwise gcd until the distinct retained block values are
pairwise coprime, while retaining multiplicity and endpoint-occurrence
provenance. Here the endpoint decompositions are
...
Thus the complete joint refinement includes the pairwise-coprime unit blocks
...
For a run whose full encoded transcript has quasipolynomial bit length in
`n`, all modular powers, pairwise gcds, factor-first alignments, and integer
normal forms take deterministic quasipolynomial time. The full encoding
includes all named blocks, the explicit retained bank, all retained
relations, complete word and occurrence provenance, and every exponent and
coefficient bit. A repeated run must impose an explicit quasipolynomial cap
on this full encoding. A cap on `kappa` or on retained-bank cardinality alone
is not sufficient: collision calls can add blocks, relations, provenance,
and coefficient bits without increasing `kappa`. The theorem does not give
a compressed evaluator for a larger bank.

The following are three alternative forms of progress that would address the
...
These are alternatives; they are not claimed to be equivalent.
