# F242 V2 candidate — an unfactored word gives an exact Miller law in every quadratic-torus orientation
...
The four-interface theorem alone supplies no all-input progress guarantee.
For a fixed word, every clean powered law is already one of these four laws,
and repeated discriminants in one orientation repeat the same residual law.
The theorem does not prove that any of the four values
`min(r_p,r_q)` is quasipolynomial.

If `W` is allowed to depend materially on the full value of `D`, that
dependence is a new integer-source proposal, not a consequence of the
four-orientation torus law.  It requires its own all-input progress proof.
