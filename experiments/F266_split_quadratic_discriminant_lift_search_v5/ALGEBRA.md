# F266-D04 algebra — canonical split-quadratic discriminant lifts
...
F266-D04 is a narrow repair of the immutable failed F266-D03 packet. D01,
D02, D03, and their hostile FAIL audits remain unchanged. D04 keeps the same
eight base families, five generators, word caps, two lift levels, direct
tickets, decoys, and P66 decoder. It changes no searched algebraic family.
...
The canonical-base stage screens `gcd(x,N)` and `gcd(Delta_0,N)`. The first
screen also proves that every admitted discriminant row is a unit modulo `N`.
...
For each public base ID, D04 chooses one gauge triple
`(g_a,g_r,g_s) in [0,N)^3` before it selects or evaluates any matrix. The
level-`N^2` source is fixed as

\[
 (a+Ng_a,\ r+Ng_r,\ s+Ng_s).
\]

Every matrix in that base uses this same triple. A word never changes the
source form or its exact base discriminant.
...
For two matrices in the same fixed base and lift level,

\[
  {R_M-R_L\over m}=\kappa_M-\kappa_L               \tag{7}
\]

is exact because their `Delta_0` is identical. For rows at different lift
levels but with the same base residues,
...
1. `source`: `gcd` of each public source candidate `a`, `r`, `s`, and `r-s`,
   plus the inverse-family seed `u`, as each candidate is attempted;
2. `base`: `gcd` of canonical base coefficients `A0,B0,C0`, exact base
   discriminant `Delta0`, and supplied discriminant root `x` for each
   retained base/lift level;
3. `transformed`: `gcd` of each transformed `A,B,C,D_M`;
4. `projective`: `gcd` of every projective coordinate, own-root determinant,
   and cross-row projective determinant;
5. `resultant`: `gcd` of the exact Sylvester resultant for every retained
   pair;
6. `carry`: `gcd` of every invariant carry, same-level quotient, and
   cross-level quotient;
7. `singleton`: both signed normalized-root gcds for every exact singleton
   square, whether base or orbit; and
8. `equal_or_template`: both signed normalized-root gcds for every equal-row
   or exact square-multiple pair.
...
D04 executes these as eight global stages. It does not interleave a later
stage for one row or pair with an earlier stage for another. A failed exact
divisibility or invariant assertion aborts the complete process. It is never
converted into an ineligible bank or a null observation.

D04 retains four counters for each stage: total gcd tests, unit gcds, full
gcds, and proper gcds. The four values obey
`tests = unit + full + proper`. Singleton and two-row normalized-root screens
contribute both signed gcd tests. These counters authenticate the complete
direct-screen population even when no proper gcd occurs.
...
- swapped base roots;
- equal positive rows, after their exact normalized-root comparison is
  labelled `EQUAL_ROW_DECOY`;
- exact square rows;
...
An equal row is not counted as a square-multiple template. A useful equal-row
comparison is still a valid direct factor certificate, but it keeps the
`EQUAL_ROW_DECOY` structural label and cannot improve the template score.
...
D04 serializes one residual row map per eligible bank and every final block
value, complete sparse exponent vector, exact-square flag, and status. Thus a
kernel certificate can be replayed without factoring an opaque block.
...
For every eligible bank with `k` distinct positive integer row values tested
before exact-square and rational-square-class removal, the report includes
...
Equal integer values count once. Ineligible and resource-rejected banks
contribute no comparison mass. This is the P213 useful-square mass for `k`
independent uniform principal
