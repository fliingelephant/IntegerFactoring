# F153 V3 candidate — Jacobi-torus orientation is one character, but it does not determine a decorated section

## Status

This is a second repaired proof-only kill-first candidate. It is not a
factoring algorithm. V1 failed hostile audit because it inferred that one
orientation character uniquely determines an F152 decorated section. V2
removed that inference. Its statement-only reconstruction found four literal
quotient and scope defects. V3 repairs only those four wordings.
...
\[
h_1h_2^{-1}:W\longrightarrow R_N/\{+1,-1\},
\qquad
...
side of a Jacobi-minus-one torus. On the split side, the standard algebraic
coordinate splitting of `A_D` needs a square root of `D`; on the nonsplit
side no high-order homomorphic image exists. This makes no claim that an
abstract isomorphism of finite cyclic point groups needs that coordinate
square root.
...
Chebyshev relations in `x` lose `D`. Choosing `t=f(alpha)` does not by itself
show that the resulting map is homomorphic or non-homomorphic. Without a
separate homomorphism or order argument, the ordinary order certificate gives
no torus-order theorem for it.
...
biquadratic coordinates before relative norm. It does not prove that this is
the unique or absolutely smallest live algebra. A torus-native large-order
