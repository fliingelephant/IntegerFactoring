...
**Status:** corrected proof-only candidate after two exact earlier versions
failed hostile audit. No hostile audit, proof-blind reconstruction, research
computation, cross-family audit, human audit, or literature audit has run on
this version. Both failed versions and audits are preserved. This is an exact
accounting theorem for multiplicative feedback and prime saturation. It is
not a feedback source law or a factoring algorithm.
...
Thus there are two exact gates for creating new known saturation
dependencies: multiplicity refinement and column closure. Accumulating many
canonical-inverse relations changes prime saturation only when one of these
gates opens. Relation count and abstract subgroup size are not valid progress
measures.
...
be pairwise-coprime integers greater than one that are units modulo \(N\).
Suppose
...
be the final pairwise-coprime list of integer blocks \(b_r>1\). Every old
block has an exact exponent factorization
...
nonclosing column forces \(a=0\), which gives (9). Its addition increases
...
For feedback saturation, the exact bookkeeping is polynomial in the total
bit size of the explicit maintained presentation and scanned primes, plus
\(\log N\). It is polynomial in \(\log N\) only when the counts and bit sizes of
retained blocks, relations, scanned primes, and feedback steps have an
explicit polynomial cap.

After each gcd-free refinement, apply the full exponent map
\(E'=\Delta_\alpha E\) and update a kernel basis modulo every scanned small
prime. Run the applicable complete decoder on a polynomial-size basis or
support-two menu; do not enumerate all vector-space directions. Then append
...
computational lower bound. It also gives no all-input polynomial state cap;
that cap is a separate requirement of any proposed closure algorithm.
