...
This is the revised proof-only candidate after the V1 statement failed its
proof-blind reconstruction because one asymptotic sentence omitted its rank
scope. It is not a factoring algorithm. This V2 statement has not passed a
...
For \(M\) such probes that are fresh, including probes that are conditionally
fresh after an adaptive transcript, a union bound gives
...
then the total mismatch probability is \(2^{-\Omega(n)}\). In particular,
this holds when both \(M\) and the explicit algebra rank \(d\) are
...
rank sampling with quasipolynomially many probes in explicit
quasipolynomial-rank algebras, or low-order characteristic jets on balanced
semiprimes. It also shows the exact hidden congruence that a monomial
replacement for the unknown local Frobenius must satisfy; it does not prove
that no menu can satisfy it.
