# F214 candidate — the full inverse box is the factor pair, while explicit representations remain exponential

## Status

This is a frozen, self-audited, proof-only candidate. It gives an exact
collapse at the full modulus and cardinality or theorem-range boundaries for
six named explicit methods. It is not a numerical-QP inverse-box algorithm,
not a general lower bound, and not an all-input factoring algorithm.
...
Factoring gives the point, and any returned point factors \(N\).
...
### 3. Correct recursive consequence

Suppose a uniform subroutine, given \(N\) and the complete factorization of
\(K\), returns the point in (3) in numerical-QP bit complexity. Recursively
factoring \(K\) creates only one near-size child: \(K\) has at most \(n-1\)
bits. The other standard child \(E\), and a numerical-QP number of children
of comparable size, have at most \(n/2+O(1)\) bits. The resulting accounting
has the P183 form

\[
T(n)\leq T(n-1)+Q(n)T(n/2+O(1))+Q(n),
\tag{6}
\]

and is numerical QP. There are not two independent near-size recursive
children.
