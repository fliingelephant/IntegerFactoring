# F270-D03 prelaunch manifest

## Classification and lineage

`F270-D03` is a post-hoc discovery analysis. It reuses only public F268-D04
...
was rejected only for the process-firewall blocker in the D02 hostile audit,
SHA-256
`1a5b62fec6ab10c411a1f60270f866dd9bd6d8a3ab8b40919032758344c23739`.
D03 keeps the D02 analyzer's arithmetic and resource code unchanged. Its only
analyzer edit changes the serialized packet-version label. D03 otherwise
changes only the runner's process firewall plus the documentation and evidence
fields needed to authenticate it.
...
compilation. It enumerates the runner's exact `(PID,starttime,PPID)` parent
chain through PID 1 and separately identifies the direct-child deadline
watchdog. It creates two complete numeric-PID-sorted `/proc` snapshots. Each
records command-line arguments, executable path, and working directory and is
capped at 1 MiB. PID lists must remain identical before, between, and after the
scans, and the snapshots must be byte-identical. Every read, identity,
consistency, count, depth, or byte-cap failure aborts. Every nonexcluded
`remote_run` or `remote_run.sh` identity aborts independently of F-number,
mode, argument position, or path spelling. The semantic production-mode and
workload checks remain. Both snapshots, exact exclusions, and literal PASS
are retained and hashed. This scan occurs before any workload child exists.
The exclusive lock is then the authoritative non-overlap mechanism for the
full packet lifetime.
...
manifest that binds both archive hashes, and `F270-D03.final.sha256` over all
