...
work_dir=/root/IntegerFactoring_F260_V2/F260-D02
...
write_runner_manifest() {
  ...
  if (( status == 1 && shell_status != 0 )); then status=$shell_status; fi
  {
    ...
    find "$out_dir" "$log_dir" -type f ! -name F260-D02.runner.manifest \
      -print0 2>/dev/null | sort -z | xargs -0 -r sha256sum 2>/dev/null || true
  } >"$log_dir/F260-D02.runner.manifest"
}
...
needles = tuple(x.encode() for x in (
    "F258-D01", "F259-D01", "F260-D01", "F261-D01",
...
verify_phase_manifest() {
  ...
  if [[ "$path" == *discovery.manifest.tsv ]]; then
    ...
    expected=$(printf '%s\n' \
      ...
      F260-D02.heldout.lead_gate.tsv | sort)
    [[ "$listed" == "$expected" ]]
  fi
}
...
programs=$(awk -F '\t' 'NR>1 && $1 ~ /^[0-9]+$/ {n++} END {print n+0}' \
  "$out_dir/F260-D02.grammar.tsv")
(( programs >= 32 ))
check_tsv "$out_dir/F260-D02.grammar.tsv" 7 $((programs+2))
check_tsv "$out_dir/F260-D02.discovery.corpus.tsv" 4 8961
[[ $(wc -l <"$out_dir/F260-D02.discovery.corpus.sha256") -eq 1 ]]
check_tsv "$out_dir/F260-D02.discovery.aggregates.tsv" 41 $((programs*9+1))
check_tsv "$out_dir/F260-D02.discovery.certificates.tsv" 6
check_tsv "$out_dir/F260-D02.discovery.collisions.tsv" 11
check_tsv "$out_dir/F260-D02.discovery.oracle.tsv" 5 7
check_tsv "$out_dir/F260-D02.selection.tsv" 11 33
[[ $(wc -l <"$out_dir/F260-D02.selection.sha256") -eq 1 ]]
verify_phase_manifest "$out_dir/F260-D02.discovery.manifest.tsv" 9

read -r selection_hash selection_label selection_extra \
  <"$out_dir/F260-D02.selection.sha256"
read -r corpus_hash corpus_label corpus_extra \
  <"$out_dir/F260-D02.discovery.corpus.sha256"
[[ "$selection_hash" =~ ^[0-9a-f]{64}$ &&
   "$selection_label" == F260-D02.selection.tsv && -z "${selection_extra:-}" ]]
[[ "$corpus_hash" =~ ^[0-9a-f]{64}$ &&
   "$corpus_label" == F260-D02.discovery.corpus.tsv && -z "${corpus_extra:-}" ]]
[[ $(sha256sum "$out_dir/F260-D02.selection.tsv" | cut -d' ' -f1) ==
   "$selection_hash" ]]
[[ $(sha256sum "$out_dir/F260-D02.discovery.corpus.tsv" | cut -d' ' -f1) ==
   "$corpus_hash" ]]
...
if run_monitored heldout "$remaining" "$log_dir/heldout.stdout" \
    ...
    "$out_dir/F260-D02.selection.tsv" "$selection_hash" \
    "$out_dir/F260-D02.discovery.corpus.tsv" "$corpus_hash" 8 full; then
  :
else
  final_status=$?
  exit "$final_status"
...
complete_expected=$(printf '%s\n' "$discovery_expected" \
  ...
  F260-D02.heldout.manifest.tsv | sort)
exact_output_set "$complete_expected" || {
  echo 'REFUSE_HELDOUT_OUTPUT_SET' >&2
  final_status=80
  exit 80
}
check_tsv "$out_dir/F260-D02.heldout.corpus.tsv" 4 24577
check_tsv "$out_dir/F260-D02.heldout.aggregates.tsv" 41 385
check_tsv "$out_dir/F260-D02.heldout.certificates.tsv" 6
check_tsv "$out_dir/F260-D02.heldout.collisions.tsv" 11
check_tsv "$out_dir/F260-D02.heldout.lead_gate.tsv" 43 33
verify_phase_manifest "$out_dir/F260-D02.heldout.manifest.tsv" 5
...
echo "F260_D02_RUNNER_PASS projected_seconds=$projection combined_bytes=$combined"
exit 0
