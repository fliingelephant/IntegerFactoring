...
Screen t and both positive children q,t-q by gcd with N. Return a verified
proper divisor if one is found. Otherwise fail if a child is zero. If both
are positive, select q or t-q with one independent fair bit. Stop with
failure when the selected child is at most one. For the first proposed
experiment, a is retained for the whole attempt.
...
Let w(t) be the cost of the reached query, with the exact early-return
priority preserved. Then
...
It can change charged early-return work if ordered screens terminate at
different positions; retain the actual order when comparing costs.
