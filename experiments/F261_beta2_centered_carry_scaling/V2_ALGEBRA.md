# F261-D02 V2 exact algebra — beta-two prefixes filter centered-carry banks

## Scope

This file fully states the V2 proof-only theorem. The finite search is
separate evidence. The V2 source computes true-carry diagnostics and exact
candidate-count upper bounds. It does not enumerate or execute this theorem's
factor bank. No statement here computes a beta-two carry prefix or proves an
...
also follows. The theorem and the exact cost bound use (5). The V2 finite
scan computes the true hidden centers and does not enumerate these public
center intervals.

## 3. A known dyadic factor prefix filters the carry bank

For the no-prefix schedule, set

\[
R=1,\qquad a=0,\qquad b=0.                            \tag{6a}
\]

This is an explicit convention. It makes no modular-inverse call.

For every nontrivial supplied prefix, let

\[
R=2^t\mid B,\qquad R\ge2,
\qquad a=p\bmod R,
\qquad b=q\bmod R=N a^{-1}\bmod R.                   \tag{6b}
\]

For `R>=2`, `a` is odd and is a unit modulo `R`, so the inverse in (6b)
exists. The values `a,b` are operational data once one beta-two prefix is
supplied. They do not use the hidden labels after `a` is known. For `R=1`,
all congruences below use the unique residue class modulo one.
...
The V2 experiment evaluates the upper bound from (10). It does not execute
(9), the quadratic or linear decoder, or candidate verification on cohort
rows. Thus no V2 `oracle_hit_*` field is a factor-bank hit.
...
surrogate for the two true centers. F261-D02 searches for such patterns on
frozen finite cohorts. Every true-carry cap event and every feature using
`p,q,K_p,K_q,x,y,c` is marked `oracle` and cannot be used as an algorithmic
input or reported as an executed factorization.
