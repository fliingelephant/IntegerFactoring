# F177 V2 candidate — one exact common order lifts to a square-modulus terminal screen

## Status and scope

This is a proof-only positive theorem for balanced squarefree semiprimes. It
improves the terminal threshold for one exact ordinary or one quadratic-torus
common order. It is not an all-input source theorem and is not a factoring
algorithm by itself.
...
## 2. Orientation-free torus common-order lift

Suppose a public certified quadratic-torus state has common order \(B\).
There is one hidden sign \(\epsilon\in\{+1,-1\}\) for which
...
The algorithm does not need to know that sign. Put \(D=q-p\). For the true
sign,
...
For each of the two public signs \(\delta\in\{+1,-1\}\), enumerate the
integers \(D\) in (13) satisfying
...
For each candidate, test whether
...
is an integer square and use (7). The true sign is among the two scans. The
combined list has at most
...
candidates. Hence the orientation-free torus channel is deterministic QP
whenever
...
Under (17), the combined count is at most \(2Q(n)+2\).
...
cost. No hidden factor or hidden orientation is used by either scan.

## 4. Exact boundary

F177 V2 improves the terminal use of one already certified common order. It
...
F177 V2 does not prove any of the following.

- A QP source reaches the threshold (9) or (17) on every input.
