# F260-D03 V3 preregistration — repaired Las Vegas integer symbolic search

## Decision boundary

This packet replaces no D01 or D02 byte. Both predecessors and both hostile
FAIL audits remain immutable. D03 is a new narrow frozen repair and needs an
independent hostile pre-run PASS before launch.

D03 changes only canonical syntax ordering, duplicate representatives,
experiment labels, residual-log extrema output, and their runner gates. The
D02 score formulas, admitted grammar scope, master seed, cohorts, ranking
orders, selection rule, lead gates, and resource limits are unchanged.
...
D03 has 22 named base sequences:
...
names. Hard-coded D03 syntax depth is at most two.

First choose the lexicographically first complete normalized syntax for each
duplicate semantic fingerprint. This choice is independent of depth and
syntax length. Then sort every candidate type by
...
F260-D03.heldout.manifest.tsv
```

Word aggregate rows contain mean, minimum, and maximum `log2(r_p)` and
`log2(r_q)`. Non-word rows use zero in the four extrema fields. Certificate
output preserves split, bit size, cohort, row index, source
...
V3 changes D01's unrealized “event” wording to an exact expected-certificate
...
4. refuse active F258-D01, F259-D01, F260-D01, F260-D02, F261-D01,
   F262-D01, F263-D01, or F264-D01 production before validation, discovery,
   and held-out;
...
At static freeze, D03 has not been compiled, executed, self-tested,
benchmarked, or transferred. `VALIDATION_PENDING.md` is part of the frozen
packet. No discovery or held-out corpus may start until target validation
passes and a fresh independent hostile audit gives an unqualified PASS.
