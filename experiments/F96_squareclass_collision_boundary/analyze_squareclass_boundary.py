...
def collision_with_first(
    ...
    return {
        **candidate,
        ...
        "same_relation_value_as_first": candidate["P"] == first_product,
...
def main() -> int:
    ...
    for label, bound in (("n", bit_length), ("n_squared", bit_length * bit_length)):
        ...
        menu_results[label] = {
            ...
            "distinct_relation_value_collision_count": sum(
                not bool(record["same_relation_value_as_first"])
                for record in menu_collisions
            ),
            ...
            "first_distinct_relation_value_collision": next(
                (
                    record
                    for record in menu_collisions
            ...
            "first_useful_same_square_class_collision": next(
                (record for record in menu_collisions if bool(record["useful"])),
                None,
            ),
        }
    ...
    output = {
        ...
        "subgroup": {
            ...
            "complete_residue_scan_same_class_count": len(subgroup_collisions),
            "complete_residue_scan_same_class_fraction": (
                f"{len(subgroup_collisions)}/{len(words)}"
            ),
            "complete_residue_scan_distinct_relation_value_count": sum(
                not bool(record["same_relation_value_as_first"])
                for record in subgroup_collisions
            ),
            "distinct_same_class_relation_values": len(
                {int(record["P"]) for record in subgroup_collisions}
            ),
            "complete_residue_scan_useful_count": sum(
                bool(record["useful"]) for record in subgroup_collisions
            ),
            "first_complete_residue_scan_same_class": (
                subgroup_collisions[0] if subgroup_collisions else None
            ),
            "complete_residue_scan_same_class_records": subgroup_collisions,
        },
        "direct_integer_feedback": {
            ...
            "single_relation_capacity_same_class_count": len(capacity_collisions),
            "single_relation_capacity_distinct_relation_value_count": sum(
                not bool(record["same_relation_value_as_first"])
                for record in capacity_collisions
            ),
        },
