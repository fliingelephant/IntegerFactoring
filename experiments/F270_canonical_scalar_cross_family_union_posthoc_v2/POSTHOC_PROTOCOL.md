# F270-D02 post-hoc protocol — canonical scalar cross-family union
...
preserves the D01 arithmetic chronology and repairs only these resource and
evidence-lifecycle defects.
...
`/tmp/integer_factoring.production.lock` for its full lifetime. It writes a
PID-sorted process snapshot capped at 1 MiB and hashes it. A generic scan of
that complete snapshot rejects any other target, discovery, held-out, search,
union, or production runner. A snapshot overflow also rejects the run. The
process-snapshot hash and literal firewall PASS enter both archived metadata
files and the final run manifest.
The prelaunch scan excludes the runner's exact ancestor chain and deadline
watchdog. It occurs before any workload child starts, so it cannot confuse the
packet's later compiler, timeout, analyzer, tar, or gzip descendants with an
external production process. The acknowledgement sidecar records the excluded
PIDs and is itself hashed into both archived metadata files and the final run
manifest. After prelaunch, the held exclusive lock is authoritative for every
cooperating research packet.
